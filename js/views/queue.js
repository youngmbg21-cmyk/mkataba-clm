...
/* ============================================================
   ...
   ============================================================ */
/* `k` is the stored contract status. The column heading is not stored beside
   it any more: statusLabel(k) reads the same dictionary entry the status chip
   uses, so a stage is named identically wherever it appears and in one place
   only. */
const PIPE_COLS=[
  ...
  return `
    <div data-card="${c.id}" class="q-card" style="background:var(--color-surface);border:1px solid var(--color-divider);border-left:4px solid ${folderColor(c)};border-radius:5px;box-shadow:var(--shadow-sm);padding:11px 12px;cursor:pointer;display:flex;flex-direction:column;gap:5px">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:6px">
        ...
        <span style="background:${rp.bg};color:${rp.fg};font-size:9.5px;font-weight:600;letter-spacing:.03em;padding:2px 8px;border-radius:999px;font-variant-numeric:tabular-nums;flex:none">${t('risk_chip',{ n:r })}</span>
      </div>
...
function pipeColumnInner(col, list){
  ...
  const more=list.length>PIPE_CAP?`<button data-pipe-more="${col.k}" style="width:100%;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:4px;padding:6px 9px;font:inherit;font-size:11px;font-weight:500;color:var(--color-accent-700);cursor:pointer">${t('queue_more',{ n:list.length-PIPE_CAP })}</button>`:'';
  const empty=list.length?'':`<div style="border:1px dashed var(--color-divider);border-radius:4px;padding:22px 10px;text-align:center;font-size:11px;color:var(--color-neutral-500)">${t('queue_empty')}</div>`;
  ...
  const columnsHtml=groups.map(g=>`
    <div style="min-width:0;display:flex;flex-direction:column;min-height:0">
      <div style="display:flex;align-items:center;gap:6px;padding:0 2px 8px;min-width:0;flex:none">
        ...
        <span style="font-family:var(--font-mono);font-weight:600;font-size:12.5px;letter-spacing:.06em;text-transform:uppercase;white-space:nowrap">${statusLabel(g.col.k)}</span>
