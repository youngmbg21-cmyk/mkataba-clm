  ...
  return `
    <div id="email-setup-banner" style="display:flex;align-items:flex-start;flex-wrap:wrap;gap:11px;border:1px solid var(--st-amber-line);background:var(--st-amber-bg);border-left:4px solid var(--st-amber-dot);border-radius:var(--radius);padding:var(--s-3) var(--s-4)">
      ...
      <span style="flex:1;min-width:0;line-height:1.5">
        ...
        <span style="display:block;font-size:var(--t-meta);color:var(--st-amber-fg);margin-top:2px">${EMAIL_SETUP_LINE}</span>
      </span>
      ${isAdmin()?`<button id="email-setup-go" class="ui-btn ui-btn-sm" style="flex:none;border-color:var(--st-amber-line)">${i18t('co_set_it_up')}</button>`:''}
    </div>`;
}
...
/* ---- TODAY, AS A DAY A MACHINE CAN READ — and `todayStr()` above is NOT it ----
   That line returns "22 Sept 2026". It is right for `c.lastAction`, which is a
   sentence on a screen, and it is WRONG anywhere a day is compared, stored or
   handed to a date box — comparing it with an ISO day compares "2" with "0".
   It sits directly above this one because that confusion has been paid for
   three times: the calendar never marked today, the wall refusing a notice
   served tomorrow refused nothing, and the risk scan's own date printed an
   em-dash for the life of every record.

   LOCAL, NEVER UTC: toISOString() would put today on yesterday's day for
   every reader west of Greenwich after their afternoon. */
const todayISO = () => {
  const d = new Date();
...
/* ---- WHO MAY MAKE NEW PAPER (Young ruled 18 Sep 2026) ----
   THE ONE READING, and it takes a person so a drawer can ask it about somebody
   else. "New paper" means paper with no approved wording behind it: writing a
   company standard template. Drafting an NDA off a template your own team
   approved is not building from scratch — it is the everyday act, and it stays
   open to anyone who can edit.

   OFF BY DEFAULT, which is the owner's own word on the row, so an absent
   answer is a no. An ADMIN always holds it: a workspace that could lock its
   own owner out of its own templates is a worse failure than the one this
   rule prevents. A VIEWER never does, because a viewer may not edit at all.

   The grant is `newPaper` on the person's own record, admin-only in both
   directions (ADMIN_ONLY_USER_FIELDS on the server strips it from a
...
function renderAuth(mode){
  ...
  const input=(id,label,type='text',ph='')=>`
    <label style="display:block;margin-bottom:14px;">
      ...
      <input id="${id}" type="${type}" placeholder="${ph}" style="width:100%;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:var(--radius);font-family:var(--font-body);color:var(--color-text);outline:none;height:var(--field-h);padding:0 var(--field-pad-x);font-size:var(--field-size)"/></label>`;
  ...
  /* min-height is WCAG 2.5.8 (target size, AA): measured at 366x17px, this is
     ...
     plain <button> put it. */
  const LINKBTN='margin-top:14px;width:100%;min-height:var(--tap-min);display:inline-flex;align-items:center;justify-content:center;background:none;border:0;font-size:var(--t-label);color:var(--color-neutral-600);cursor:pointer;font-family:var(--font-body);';
  if(mode==='setup'){
    root.innerHTML = shell(`
      ...
      <label style="display:block;margin-bottom:14px;">
        ...
        <select id="su-market" style="width:100%;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:var(--radius);font-family:var(--font-body);color:var(--color-text);outline:none;height:var(--field-h);padding:0 var(--field-pad-x);font-size:var(--field-size)">
    ...
    root.innerHTML = shell(`
      ...
      <p id="li-err" class="hidden" style="text-align:center;font-size:var(--t-meta);color:var(--st-ruby-dot);margin-top:var(--s-3);"></p>
    ...
    root.innerHTML = shell(`
      ...
      <div id="fp-result" style="margin-top:var(--s-3);"></div>
    ...
    root.innerHTML = shell(`
      ...
      <p id="rs-err" class="hidden" style="text-align:center;font-size:var(--t-meta);color:var(--st-ruby-dot);margin-top:var(--s-3);"></p>`);
  ...
  authRoot.innerHTML=`
    <div style="min-height:100vh;display:grid;place-items:center;background:var(--color-bg);padding:0 var(--s-4);">
      <div style="background:var(--color-surface);border:1px solid var(--color-divider);box-shadow:var(--shadow-lg);border-radius:var(--radius);padding:30px;max-width:25rem;width:100%;">
        ...
        <button id="cp-go" class="ui-btn ui-btn-lg ui-btn-primary" style="width:100%">${i18t('co_set_my_password')}</button>
        <p id="cp-err" class="hidden" style="text-align:center;font-size:var(--t-meta);color:var(--st-ruby-dot);margin-top:var(--s-3);"></p>
      </div></div>`;
...
function startApp(){
  ...
  setView(['dashboard','register','pipeline','advice','intake','obligations','approvals','folder','intel','calendar','reports','templates','playbook','workspace','team','directory','migration','redline'].includes(state.view)?state.view:'dashboard');
    ...
    <div class="px-5 py-4">
          ...
          <div class="rounded-lg border ${r.status==='open'?'border-gold-500/30 bg-gold-500/5':'border-brand-100 bg-white'} p-3">
            ...
            ${r.status==='open'?(canEdit()?`
              <div class="mt-2 flex items-center gap-2">
                ${r.proposedText?`<button data-nego-redline="${r.n}" class="ui-btn ui-btn-sm ui-btn-primary">${icon('history','w-3 h-3')} Review redline</button>
                <button data-nego-reject="${r.n}" class="ui-btn ui-btn-sm">${i18t('co_reject')}</button>`
                :`<button data-nego-accept="${r.n}" class="ui-btn ui-btn-sm ui-btn-primary">${icon('check2','w-3 h-3')} ${r.proposedValue!=null?i18t('co_accept_apply_value'):i18t('co_accept')}</button>
                <button data-nego-reject="${r.n}" class="ui-btn ui-btn-sm">${i18t('co_reject')}</button>`}
      ...
      ${resolvedRounds(c).length&&canEdit()&&c.status!=='Signed'?`
        <div style="display:flex;align-items:center;gap:9px;flex-wrap:wrap;margin-top:10px;border-top:1px solid var(--color-divider);padding-top:10px">
          <span style="flex:1;min-width:140px;font-size:var(--t-label);color:var(--color-neutral-700)">${i18t('co_ready_next_round')}</span>
          <button id="nego-reshare" class="ui-btn ui-btn-primary" style="flex:none">${icon('send','w-3.5 h-3.5')} ${i18t('co_send_updated')}</button>
        </div>`
...
/* ============================================================
   ...
   disagreeing flavours — two of them in this file 1,300 lines apart, on
   8px/10px against 6px/10px padding, 13px against 14px type, and one with no
   min-height at all. RV_FLD in js/review.js carried a note saying it was a
   deliberate copy to be kept in step and HAD ALREADY DRIFTED 2px from what it
   quoted. That is how a form ends up two pixels off the form beside it.

   IT READS THE FIELD TOKENS, so the copy cannot drift again even in
   principle: a change to a field's height or padding is one edit in :root and
   every form in the product follows it.

   THE LOCAL `const FLD` LINES ARE KEPT AS ASSIGNMENTS from these, rather than
   deleted and the call sites re-pointed — a builder half way through a 400-line
   ...
   one truth. */
const HATI_FLD='width:100%;height:var(--field-h);border:1px solid var(--field-line);background:var(--color-surface);border-radius:var(--radius);padding:var(--field-pad-y) var(--field-pad-x);font-size:var(--field-size);font-family:var(--font-body);color:var(--color-text);line-height:var(--field-lh);';
...
function confirmDialog(opts={}){
  ...
  return new Promise(resolve=>{
    ...
    ov.style.cssText='position:fixed;inset:0;z-index:90;display:grid;place-items:center;padding:var(--s-4)';
...
/* THE ONE-SCREEN ROW SAYS ONE LINE UNDER THE CHOSEN PURPOSE (the pop-up diet,
   ...
   and nothing about which room they land in or how their items come back.
   `line` is that sentence; `title` and `blurb` stay for the three cards of
  ...
  const card = (k, title, blurb, on, disabled) => `
    <button type="button" data-share-kind="${k}" aria-pressed="${on?'true':'false'}"${disabled?' disabled':''}
      ...
      <span style="min-width:0">
        <span style="display:block;font-size:var(--t-body);font-weight:var(--w-title);color:${on?'var(--color-accent-800)':'var(--color-text)'}">${title}</span>
        <span style="display:block;font-size:var(--t-label);line-height:1.5;color:var(--color-neutral-600);margin-top:3px">${blurb}</span>
      </span>
    </button>`;
  return `
  ...
  const btn=(k)=>{ const on=sel===k, m=SHARE_PURPOSE_COPY[k];
    return `<button type="button" data-share-purpose="${k}" aria-pressed="${on?'true':'false'}"
      ...
      <span style="display:flex;align-items:center;gap:7px">
        ...
        <span style="font-size:var(--t-body);font-weight:var(--w-title);color:${on?'var(--color-accent-800)':'var(--color-text)'}">${m.label}</span>
      </span>
      <span style="display:block;font-size:var(--t-meta);font-weight:var(--w-strong);color:var(--color-text);margin:5px 0 2px">${m.title}</span>
...
/* THE ONE NOTE BOX (the pop-up diet, 13 Sep 2026). There were two: this one on
   step 1 and "Personal message" on step 2, joined into one message before
...
function shareNoteBoxHtml(c, hist){
  return `<label style="display:block"><span id="sh-summary-label" style="display:block;font-size:var(--t-label);font-weight:var(--w-strong);color:var(--color-neutral-700);margin-bottom:var(--s-1);font-family:var(--font-heading);letter-spacing:.02em;">${
        ...
        <textarea id="sh-summary" rows="3" style="${HATI_FLD}height:auto;min-height:var(--field-h)" placeholder="${
  ...
  return `
    <div id="share-step-1"${opts.hiddenStart?' class="hidden"':''}>
      ${''/* ---- THE ORDER ON ONE SCREEN IS THE OWNER'S DRAWING ---- (the pop-up
             diet, 13 Sep 2026): title, what travels, what this round is for,
             then the recipient and the note, which live in step 2. The pieces
             below are built once each and placed by `one`: the two-screen
             shape keeps its own order. */}
      ...
      <p id="share-manifest-line"${hist?' class="hidden"':''} style="font-size:var(--t-meta);color:var(--color-neutral-700);margin:0 0 var(--s-3);line-height:1.55;">
        ${s&&s.lines.length
          ? `Round ${s.total?esc(String(s.round)):''} — ${s.lines.length} change${s.lines.length===1?'':'s'} on the table${s.pending?`, ${s.pending} still awaiting a decision`:''}.${one?'':` ${esc(c.counterparty||'The counterparty')} sees this summary alongside the link.`}`
          : (one ? i18t('co_no_changes_yet_short', { who:esc(c.counterparty||i18t('co_them')) })
      ...
      <p id="share-manifest-line-hist"${hist?'':' class="hidden"'} style="font-size:var(--t-meta);color:var(--color-neutral-700);margin:0 0 var(--s-3);line-height:1.55;">
        ${s&&s.lines.length
      ...
      <div id="share-hist-preview"${hist?'':' class="hidden"'} style="margin:0 0 14px">
        ...
        <div class="ht-inert" aria-hidden="true">${window.negoTimelineScreenHtml
          ...
          : `<div style="padding:14px;font-size:var(--t-meta);color:var(--color-neutral-600)">${i18t('co_history_not_here')}</div>`}</div>
      </div>
...
function shareSignerRowsHtml(c, sel){
  ...
  if(!plan.length) return { plan, html:`<p style="margin:0;font-size:var(--t-meta);line-height:1.55;color:var(--color-neutral-600)">${
  ...
  const html=plan.map((s,i)=>{
    ...
    return `<${pickable?'button type="button"':'div'} ${pickable?`data-share-signer="${esc(s.id)}"`:''}
      ...
      <span style="flex:1;min-width:0">
        <span style="display:block;font-size:var(--t-meta);font-weight:var(--w-strong);color:var(--color-text)">${esc(s.name||'—')}${
...
function shareSignerPickHtml(c, sel){
  ...
  return `<div id="share-signers-box" data-need-signers="${need?'1':'0'}" style="margin:0 0 14px;${box};border-radius:var(--radius);padding:11px var(--s-3)">
    ...
    <button type="button" id="share-signer-edit" class="${need?'ui-btn ui-btn-primary':'ui-btn'}" style="width:100%;justify-content:center;margin-top:${plan.length?'3px':'8px'}">${
      icon('users','w-3.5 h-3.5')} ${plan.length?i18t('co_add_reorder_signers'):i18t('co_add_signers')}</button>
    <p style="margin:7px 0 0;font-size:var(--t-label);line-height:1.5;color:${need?'var(--st-amber-fg)':'var(--color-neutral-500)'}">${
...
/* THE COUNT HAS TO COUNT THE LIST UNDER IT. This printed "1 thing to fix" —
   ...
   its own quiet line. */
function readinessPanelHtml(c, o={}){
  const probs=contractReadiness(c);
  if(!probs.length) return '';
  /* ---- ON ONE SCREEN THE WARNINGS FOLD TO A COUNT ---- (the pop-up diet,
     owner-approved 13 Sep 2026). Amber notes are facts worth a glance, not a
     wall: the head says how many and opens on a press. A BLOCK stays open —
     it carries the tick that lets the send through, and a tick behind a fold
     is a tick nobody sees. */
  if(o.fold && !probs.some(x=>x.severity==='block')){
    const n=probs.length;
    return `<div id="share-readiness" style="margin:0 0 var(--s-3);border:1px solid var(--st-amber-line);background:var(--st-amber-bg);border-radius:var(--radius);padding:8px var(--s-3);">
      <details><summary style="cursor:pointer;display:flex;align-items:center;gap:6px;font-size:var(--t-meta);font-weight:var(--w-strong);color:var(--st-amber-fg)">${icon('alert','w-3.5 h-3.5')} ${i18tn('co_worth_checking_n', n, { n })}</summary>
      <ul style="margin:6px 0 0;padding-left:var(--s-4);font-size:var(--t-meta);line-height:1.65;color:var(--st-amber-fg);">${probs.map(x=>`<li>${esc(x.label)}</li>`).join('')}</ul></details>
    </div>`;
  }
  ...
  const sendFormHtml=(pre, o={})=>`
      ...
      <div id="share-fields">
        ...
        ${''/* ---- A BOX MUST SAY WHERE WHAT YOU TYPE IN IT GOES (13 Aug 2026),
               ...
               a copied link carries no message. On the channels that carry it
               the line is empty — "it goes in the email" under an email form
      ...
      ${emailOff()?`<div id="sh-noemail" style="margin:0 0 var(--s-3);border:1px solid var(--st-amber-line);background:var(--st-amber-bg);border-radius:var(--radius);padding:9px var(--s-3);font-size:var(--t-meta);line-height:1.55;color:var(--st-amber-fg)">${i18t('co_email_off_line')}</div>`:''}
      ...
      ${''/* THE FOOT STAYS IN VIEW AND SAYS CANCEL (the Compact ladder, 26 Sep
             2026 — rule 8 of the button review). On a laptop Send sat below
             the visible edge until you scrolled inside the dialog; the foot is
             pinned to the dialog's own bottom now. And leaving before anything
             went out undoes nothing, so the word is Cancel — it becomes Close
...
`;
  ...
  const setKind = k => {
    ...
    if(lab) lab.textContent = hist
      ? i18t('co_note_with_record')
      : i18t('co_note_to', { who:c.counterparty||i18t('co_them') });
  ...
  const paintPurpose=()=>{
    ...
    if(say){ const m=SHARE_PURPOSE_COPY[purposeSel]||SHARE_PURPOSE_COPY.negotiate;
      say.innerHTML=say.hasAttribute('data-compact') ? esc(m.line)
        : `<b style="color:var(--color-text)">${m.title}</b> ${m.blurb}`; }
  ...
  const setCh=k=>{ ch=k;
    ...
    if(where){
      ...
      /* Only the warning is printed (co_note_goes_nowhere). The three
         statements — co_note_goes_email, co_note_goes_whatsapp,
         co_note_goes_with_file — are inert since the pop-up diet. */
      where.textContent=none?i18t('co_note_goes_nowhere'):'';
      where.style.color='var(--st-amber-fg)';
      where.style.fontWeight='600';
    }
  };
  ...
  const resultBox=(html)=>{ const host=document.getElementById(qsActive?'qs-result':'sh-result');
    ...
    const x=document.getElementById('share-close'); if(x) x.textContent=i18t('act_close'); };
  const copyBox=(link,note)=>`
    <div style="border:1px solid var(--color-divider);background:var(--st-steel-bg);border-radius:var(--radius);padding:var(--s-3);">
      ...
      <button id="share-copy" class="ui-btn" style="margin-top:6px">${icon('copy','w-3 h-3')} ${i18t('co_copy_link')}</button>
    </div>`;
...
function reshareNotSentModal(c, out, who){
  ...
  openModal(`
    <div style="padding:22px var(--s-6)">
      <div style="display:flex;align-items:center;gap:var(--s-2);margin-bottom:var(--s-1)"><span style="color:var(--st-amber-dot);display:inline-flex">${icon('alert')}</span>
        <h2 style="font-family:var(--font-heading);font-weight:var(--w-strong);font-size:18px;margin:0">${i18t('co_link_updated_not_sent')}</h2></div>
...
function counterpartySeenHtml(c, shares){
  ...
  return `
    <div id="seen-state" style="display:flex;align-items:center;gap:9px;flex-wrap:wrap;border:1px solid ${stale?'var(--st-amber-line)':'var(--color-divider)'};background:${stale?'var(--st-amber-bg)':'var(--color-surface)'};border-radius:var(--radius);padding:9px 13px;margin-bottom:var(--s-3);font-size:var(--t-meta);color:${stale?'var(--st-amber-fg)':'var(--color-neutral-700)'}">
      ...
      ${canEdit()?`<button id="seen-resend" class="ui-btn ui-btn-sm" style="flex:none">${i18t('co_send_again')}</button>`:''}
    </div>`;
}
...
function shareJourneyHtml(c, shares){
  ...
  const cell=s=>{
    ...
    return `<div data-journey="${s.k}" style="flex:1;min-width:108px">
      <div style="display:flex;align-items:center;gap:7px">
        ...
        <span style="font-size:var(--t-label);font-weight:${cur?'700':'600'};color:${s.on?'var(--color-text)':'var(--color-neutral-500)'}">${s.label}</span>
      </div>
  ...
  return `<div id="share-journey" style="border:1px solid ${j.stale?'var(--st-amber-line)':'var(--color-divider)'};background:${j.stale?'var(--st-amber-bg)':'var(--color-bg)'};border-radius:var(--radius);padding:11px 13px;margin-bottom:var(--s-3)">
    ...
    <div style="display:flex;align-items:center;gap:9px;margin-top:9px;padding-top:var(--s-2);border-top:1px dashed var(--color-divider)">
      ...
      ${(j.stale&&typeof canEdit==='function'&&canEdit())?`<button id="seen-resend" class="ui-btn ui-btn-sm" style="flex:none">${i18t('co_send_again')}</button>`:''}
    </div>
  </div>`;
}
      ...
      ${shares.map(s=>{
        ...
        return `<div style="border:1px solid var(--color-divider);border-radius:var(--radius);padding:var(--s-2) 10px;background:var(--color-bg)">
          <div style="display:flex;align-items:center;gap:var(--s-2);min-width:0">
            ...
            <span style="font-size:var(--t-label);color:var(--color-neutral-500);font-family:var(--font-mono);flex:none">${chLabel[s.channel]||'Link'}</span>
          </div>
