  ...
  return `
    <div data-adv-card="${r.id}" ${drag?'draggable="true"':''} class="q-card" style="background:var(--color-surface);border:1px solid var(--color-divider);border-radius:0;box-shadow:var(--shadow-sm);padding:11px 12px;cursor:${drag?'grab':'pointer'};display:flex;flex-direction:column;gap:5px">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:6px">
        <span style="font-family:var(--font-mono);font-size:11px;color:var(--color-neutral-600)">${r.id}</span>
        <span style="display:flex;align-items:center;gap:4px;flex:none">
          ${r.urgency==='priority'?`<span style="background:var(--st-amber-bg);color:var(--st-amber-fg);font-size:10px;font-weight:600;letter-spacing:.03em;padding:2px 8px;border-radius:0">${i18t('adv_priority')}</span>`:''}
          <span style="font-size:10px;font-weight:600;letter-spacing:.03em;padding:2px 8px;border-radius:0;font-variant-numeric:tabular-nums;background:color-mix(in srgb,${etaCol} 12%,#fff);color:${etaCol}">${etaTxt}</span>
        </span>
      </div>
      <div style="display:flex;align-items:center;gap:6px;font-size:13px;font-weight:400;line-height:1.3"><span style="display:inline-flex;color:var(--color-accent-700);flex:none">${icon(svc.ic,'w-3.5 h-3.5')}</span><span style="min-width:0">${esc(svc.name)}</span></div>
      ...
      ${r.contractName?`<div style="font-size:11px;color:var(--color-neutral-500);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(r.contractName)}</div>`:''}
      <div style="display:flex;align-items:center;justify-content:space-between;gap:6px;border-top:1px solid color-mix(in srgb,var(--color-text) 8%,transparent);padding-top:5px;margin-top:1px">
        ...
        ${ini?`<span title="${esc(r.assignee)}" style="width:20px;height:20px;border-radius:50%;background:var(--color-accent-200);color:var(--color-accent-800);display:inline-grid;place-items:center;font-size:9px;font-weight:700;font-family:var(--font-mono);flex:none">${ini}</span>`
...
function renderAdviceDesk(){
  ...
  const kpi=(label,val,col)=>`<div style="background:var(--color-surface);border:1px solid var(--color-divider);border-radius:0;padding:8px 14px;min-width:0">
      <div style="font-size:10px;letter-spacing:.1em;text-transform:uppercase;color:var(--color-neutral-600)">${label}</div>
...
function openAdviceModal(id){
  ...
  const notes=(r.notes||[]).slice().reverse().map(n=>`
    <div style="border:1px solid var(--color-divider);border-radius:0;background:var(--color-bg);padding:8px 10px;margin-bottom:6px">
      <div style="font-size:12px;line-height:1.5">${esc(n.text)}</div>
  ...
  openModal(`
    <div style="padding:22px 24px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:14px">
        <label style="display:block"><span style="display:block;font-size:11px;font-weight:600;color:var(--color-neutral-700);margin-bottom:4px;font-family:var(--font-mono)">${i18t('adv_assigned_counsel')}</span>
        ...
        <label style="display:block"><span style="display:block;font-size:11px;font-weight:600;color:var(--color-neutral-700);margin-bottom:4px;font-family:var(--font-mono)">${i18t('adv_stage')}</span>
      ...
      <label style="display:block;margin-top:10px"><span style="display:block;font-size:11px;font-weight:600;color:var(--color-neutral-700);margin-bottom:4px;font-family:var(--font-mono)">${i18t('adv_add_note')}</span>
...
function openRateCardModal(){
  ...
  const inp=(id,v)=>`<input id="${id}" type="number" min="1" value="${v}" ${editable?'':'disabled'} style="width:100%;border:1px solid var(--color-divider);background:${editable?'var(--color-surface)':'var(--color-bg)'};border-radius:0;padding:5px 7px;font-family:var(--font-mono);font-size:12px;color:inherit;outline:none"/>`;
  ...
  openModal(`
    <div style="padding:22px 24px">
      ...
      <div class="table-scroll"><table style="width:100%;border-collapse:collapse">
        <thead><tr style="text-align:left;border-bottom:1px solid var(--color-divider);color:var(--color-neutral-600);font-size:10px;letter-spacing:.08em;text-transform:uppercase">
      ...
      <p style="font-size:11px;color:var(--color-neutral-500);margin:10px 0 0;line-height:1.5">${i18t('adv_rate_note')}</p>
...
function openAdviceIntakeModal(){
  const inputStyle='width:100%;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:0;padding:7px 10px;font:inherit;font-size:13px;color:inherit;outline:none';
  const field=(id,label,ph,type='text')=>`<label style="display:block;margin-bottom:10px"><span style="display:block;font-size:11px;font-weight:600;color:var(--color-neutral-700);margin-bottom:4px;font-family:var(--font-mono)">${label}</span><input id="${id}" type="${type}" placeholder="${ph}" style="${inputStyle}"/></label>`;
  openModal(`
    <div style="padding:22px 24px">
      ...
      <label style="display:block;margin-bottom:10px"><span style="display:block;font-size:11px;font-weight:600;color:var(--color-neutral-700);margin-bottom:4px;font-family:var(--font-mono)">${i18t('adv_service')}</span>
      ...
      <label style="display:block;margin-bottom:10px"><span style="display:block;font-size:11px;font-weight:600;color:var(--color-neutral-700);margin-bottom:4px;font-family:var(--font-mono)">${i18t('adv_what_need')}</span>
