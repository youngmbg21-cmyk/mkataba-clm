...
async function renderCompanyTemplatesSection() {
  ...
  try {
    ...
    host.innerHTML = `<section style="background:var(--color-surface);border:1px solid var(--color-divider);border-radius:0;padding:16px;font-size:13px;color:var(--st-ruby-fg)">${i18t('tl_load_failed',{err:esc(e.message)})}</section>`;
    return;
  }
  if (typeof updateSidebarCounts === 'function') updateSidebarCounts();
...
function tplConfirmPaint() {
  const s = _tplConfirm;
  const CARD = 'background:var(--color-surface);border:1px solid var(--color-divider);box-shadow:var(--shadow-sm);border-radius:0';
  const INP = 'border:1px solid var(--color-divider);background:var(--color-surface);border-radius:0;padding:5px 8px;font:inherit;font-size:13px;outline:none';
  ...
  openModal(`
    <div style="padding:20px 22px;max-width:460px">
      ...
      <label style="display:block;margin-bottom:10px"><span style="display:block;font-size:12px;font-weight:600;margin-bottom:4px">${i18t('tl_name')}</span>
        <input id="tpllib-name" style="width:100%;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:0;padding:7px 10px;font:inherit;font-size:14px;outline:none" placeholder="e.g. Account Opening Form" maxlength="160"></label>
      <label style="display:block;margin-bottom:10px"><span style="display:block;font-size:12px;font-weight:600;margin-bottom:4px">${i18t('tl_category')}</span>
        <select id="tpllib-cat" style="width:100%;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:0;padding:7px 10px;font:inherit;font-size:14px;outline:none"><option value="other">${i18t('tl_other_category')}</option>${cats}</select></label>
      ...
      <label style="display:block;margin-bottom:10px"><span style="display:block;font-size:12px;font-weight:600;margin-bottom:4px">${i18t('tl_stream')}</span>
        <select id="tpllib-stream" style="width:100%;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:0;padding:7px 10px;font:inherit;font-size:14px;outline:none">
      ...
      <label style="display:block;margin-bottom:16px"><span style="display:block;font-size:12px;font-weight:600;margin-bottom:4px">${i18t('tl_description')} <span style="font-weight:400;color:var(--color-neutral-500)">(optional)</span></span>
        <textarea id="tpllib-desc" style="width:100%;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:0;padding:7px 10px;font:inherit;font-size:14px;outline:none;min-height:60px" maxlength="2000" placeholder="${i18t('tl_what_for')}"></textarea></label>
      <div style="display:flex;justify-content:flex-end;gap:8px">
        <button class="ui-btn" onclick="closeModal()">${i18t('act_cancel')}</button>
...
async function openTemplateLibDetail(id) {
  ...
  <div class="view-enter" style="padding:16px 18px 28px;display:flex;flex-direction:column;gap:14px;max-width:860px">
    ...
    <section style="${CARD};padding:18px">
      ...
      ${t.status === 'archived' ? `<p style="margin:12px 0 0;font-size:12px;color:var(--color-neutral-600);background:var(--color-neutral-100);border-radius:0;padding:8px 12px">
...
function tplLibMetaModal(t) {
  ...
  openModal(`
    <div style="padding:20px 22px;max-width:460px">
      ...
      <label style="display:block;margin-bottom:10px"><span style="display:block;font-size:12px;font-weight:600;margin-bottom:4px">${i18t('tl_name')}</span>
        <input id="tpllib-m-name" style="width:100%;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:0;padding:7px 10px;font:inherit;font-size:14px;outline:none" maxlength="160" value="${esc(t.name)}"></label>
      <label style="display:block;margin-bottom:10px"><span style="display:block;font-size:12px;font-weight:600;margin-bottom:4px">${i18t('tl_category')}</span>
        <select id="tpllib-m-cat" style="width:100%;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:0;padding:7px 10px;font:inherit;font-size:14px;outline:none">${cats}</select></label>
      ${''/* ---- WHICH STREAM IT IS FILED UNDER (15 Aug 2026, OI-11) ----
             The draft-from-template picker opens on the value streams now, so a
             template needs one or it lands in "Other". Optional on purpose: an
             unfiled template is an honest state, and "Other" is where it goes
             ...
             the product reads. */}
      <label style="display:block;margin-bottom:10px"><span style="display:block;font-size:12px;font-weight:600;margin-bottom:4px">${i18t('tl_stream')}</span>
        <select id="tpllib-m-stream" style="width:100%;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:0;padding:7px 10px;font:inherit;font-size:14px;outline:none">
      ...
      <label style="display:block;margin-bottom:16px"><span style="display:block;font-size:12px;font-weight:600;margin-bottom:4px">${i18t('tl_description')}</span>
        <textarea id="tpllib-m-desc" style="width:100%;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:0;padding:7px 10px;font:inherit;font-size:14px;outline:none;min-height:60px" maxlength="2000">${esc(t.description)}</textarea></label>
      <div style="display:flex;justify-content:flex-end;gap:8px">
        <button class="ui-btn" onclick="closeModal()">${i18t('act_cancel')}</button>
...
function tplFormInputHtml(f, value, idx) {
  ...
  const INP = 'width:100%;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:0;padding:6px 9px;font:inherit;font-size:13px;outline:none';
...
function tplFormPopover(c, idx, anchor) {
  ...
  pop.id = 'tplf-pop';
  pop.style.cssText = `position:fixed;z-index:80;top:${Math.round(r.bottom + 6)}px;left:${Math.round(Math.min(Math.max(8, r.left), (window.innerWidth || 1200) - 296))}px;width:284px;background:var(--color-surface);border:1px solid var(--color-divider);border-radius:0;box-shadow:var(--shadow-md);padding:10px 12px`;
  pop.innerHTML = `
