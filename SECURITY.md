...
- **Server-mediated in server mode; browser-direct in local mode — by design, for now.** In server (hosted) mode, all Copilot calls (the main chat panel and the Intel dock) go through the app's own `/api/ai/*` endpoints and the provider key lives in server settings / `ANTHROPIC_API_KEY` — the browser never sees it. In **static/local mode** there is no HaTi server, so the Copilot calls the Anthropic API **directly from the browser** with a key the admin saved in that browser (BYOK, single-user, own data). That local mode exists today by design and is scheduled for retirement when the key moves fully server-side.
- **Grounded and cited — and quotes are verified.** Copilot answers from tool-fetched workspace data and cites the contracts it used. The `/api/ai/chat` tool loop reads contract records, findings, and search results from the database rather than free-associating, which is also the main mitigation against prompt injection. Every verbatim quote in a citation is checked against the cited contract's actual text (typography- and whitespace-tolerant) before it reaches the browser; a quote that cannot be matched is removed, the response says so, and the citation keeps only the contract id.
- **Every Copilot exchange is logged.** Each completed `/api/ai/chat` turn — including provider errors and gave-up fallbacks — writes a row to the server-side `copilot_log` table (who asked, what was asked, the answer, the contracts cited, the tools used, the model, and any quote drops). Admins read it via `GET /api/ai/log` (org-scoped, newest first). Retention/purge policy is an open decision.
