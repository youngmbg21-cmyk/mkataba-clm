...
const advicePortalShell = inner => `
  <div style="min-height:100vh;background:var(--color-bg);">
    ...
    <div style="max-width:1100px;margin:0 auto;padding:28px var(--s-6);">${inner}</div>
    <p style="max-width:1100px;margin:0 auto;padding:0 var(--s-6) 28px;font-size:var(--t-label);color:var(--color-neutral-500);line-height:1.6">
...
function renderAdviceTracking(r){
  ...
  const steps=path.map((k,i)=>{
    ...
    return `
      <div style="display:flex;gap:var(--s-3);position:relative">
        ...
        <div style="padding-bottom:${i<path.length-1?'18px':'0'};min-width:0">
          <div style="font-size:var(--t-body);font-weight:var(--w-strong);color:${state_==='todo'?'var(--color-neutral-500)':'var(--color-text)'}">${s.label}${state_==='now'?` <span style="font-size:var(--t-label);font-weight:var(--w-strong);color:${s.color}">${i18t('apo_current_stage')}</span>`:''}</div>
          <div style="font-size:var(--t-label);color:var(--color-neutral-600);line-height:1.45">${s.desc}</div>
          ${hit?`<div style="font-size:var(--t-label);color:var(--color-neutral-500);font-family:var(--font-mono);margin-top:2px">${fmtDT(hit.at)}</div>`:''}
        </div>
      </div>`;
  }).join('');
