...
function adviceCard(r){
  ...
  const etaTxt = done ? (r.status==='Delivered'?t('advice_eta_delivered'):t('advice_eta_closed'))
    : left==null ? t('value_none')
    : left<0 ? t('advice_eta_overdue',{n:-left})
    : left===0 ? t('advice_eta_today') : t('advice_eta_left',{n:left});
  const etaCol = done ? 'var(--color-neutral-500)' : left!=null&&left<0 ? '#b0453c' : left!=null&&left<=1 ? '#7d5a14' : 'var(--color-neutral-600)';
  const q=r.quote||{};
  const fee=q.rate?t('advice_fee_range',{min:fmtKESshort(q.rate*q.hoursMin),max:fmtKESshort(q.rate*q.hoursMax)}):t('value_none');
    ...
    <div data-adv-card="${r.id}" ${drag?'draggable="true"':''} class="q-card" style="background:var(--color-surface);border:1px solid var(--color-divider);border-radius:5px;box-shadow:var(--shadow-sm);padding:11px 12px;cursor:${drag?'grab':'pointer'};display:flex;flex-direction:column;gap:5px">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:6px">
        ...
        <span style="display:flex;align-items:center;gap:4px;flex:none">
          ${r.urgency==='priority'?`<span style="background:#fbf4e3;color:#7d5a14;font-size:9.5px;font-weight:600;letter-spacing:.03em;padding:2px 8px;border-radius:999px">${t('advice_priority')}</span>`:''}
      ...
      <div style="display:flex;align-items:center;gap:6px;font-size:12.5px;font-weight:500;line-height:1.3"><span style="display:inline-flex;color:var(--color-accent-700);flex:none">${icon(svc.ic,'w-3.5 h-3.5')}</span><span style="min-width:0">${esc(adviceServiceName(svc))}</span></div>
      <div style="font-size:11px;color:var(--color-neutral-600);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${r.company?t('advice_party',{name:esc(r.name),company:esc(r.company)}):esc(r.name)}</div>
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;gap:6px;border-top:1px solid rgba(29,31,32,.07);padding-top:5px;margin-top:1px">
        ...
        ${ini?`<span title="${esc(r.assignee)}" style="width:20px;height:20px;border-radius:50%;background:var(--color-accent-200);color:var(--color-accent-800);display:inline-grid;place-items:center;font-size:8.5px;font-weight:700;font-family:var(--font-mono);flex:none">${ini}</span>`
            :`<span style="font-size:10px;color:var(--color-neutral-400)">${t('advice_unassigned')}</span>`}
      </div>
    </div>`;
}
...
function renderAdviceDesk(){
  ...
  const columnsHtml=groups.map(g=>`
    <div style="min-width:0;display:flex;flex-direction:column;min-height:0">
      <div style="display:flex;align-items:center;gap:6px;padding:0 2px 8px;min-width:0;flex:none">
        ...
        <span style="font-family:var(--font-mono);font-weight:600;font-size:12px;letter-spacing:.06em;text-transform:uppercase;white-space:nowrap">${t(g.col.lk)}</span>
      ...
      <div data-adv-drop="${g.col.k}" class="pipe-col scroll-thin" style="background:rgba(89,128,166,.05);border:1px solid var(--color-divider);border-radius:8px;padding:8px;display:flex;flex-direction:column;gap:8px;flex:1;min-height:0;overflow-y:auto">
        ${g.list.map(adviceCard).join('')||`<div style="border:1px dashed var(--color-divider);border-radius:4px;padding:22px 10px;text-align:center;font-size:11px;color:var(--color-neutral-500)">${t('queue_empty')}</div>`}
      </div>
    </div>`).join('');
  ...
  <div class="view-enter" style="height:calc(100vh - 52px);box-sizing:border-box;padding:14px 16px 18px;display:flex;flex-direction:column;gap:12px">
    ...
    <div style="display:flex;align-items:stretch;gap:10px;flex:none;flex-wrap:wrap">
      ${kpi(t('advice_kpi_active'),active.length)}
      ${kpi(t('advice_kpi_due48'),dueSoon,dueSoon?'#7d5a14':undefined)}
      ${kpi(t('advice_kpi_overdue'),overdue,overdue?'#b0453c':undefined)}
      ${kpi(t('advice_kpi_delivered30'),delivered30,'#1e6b4d')}
      ${kpi(t('advice_kpi_projected'),fmtKESshort(projected))}
      <span style="flex:1"></span>
      <div style="display:flex;align-items:center;gap:8px;flex:none">
        <button id="adv-rates" class="ui-btn">${icon('coins','w-3.5 h-3.5')} ${t('advice_btn_rates')}</button>
        <button id="adv-link" class="ui-btn">${icon('share','w-3.5 h-3.5')} ${t('advice_btn_intake')}</button>
        ${canEdit()?`<button id="adv-new" class="ui-btn ui-btn-primary">${icon('plus','w-3.5 h-3.5')} ${t('advice_btn_new')}</button>`:''}
      </div>
    </div>
...
async function adviceMove(id, target){
  ...
  if(!canEdit()){ toast(t('advice_toast_viewer_no_move'),'err'); return; }
  if(r.status===target) return;
  if(target==='Closed' && !await confirmDialog({title:t('advice_dlg_close_title',{id:r.id}), message:t('advice_dlg_close_msg'), confirmLabel:t('advice_dlg_close_confirm'), danger:true})) return;
  try{
    await updateAdviceRequest(id,{status:target});
    toast(t('advice_toast_moved',{id:r.id,stage:adviceStageLabel(target)}), target==='Closed'?'err':'ok');
...
function wireAdviceBoard(){
  ...
  document.getElementById('adv-link')?.addEventListener('click',async()=>{
    ...
    try{ await navigator.clipboard.writeText(link); toast(t('advice_toast_link_copied')); }
    catch(e){ openModal(`<div style="padding:22px 24px"><h2 style="font-family:var(--font-heading);font-weight:600;font-size:17px;margin:0 0 8px">${t('advice_modal_link_title')}</h2><textarea readonly rows="2" style="width:100%;border:1px solid var(--color-divider);background:var(--color-bg);border-radius:4px;padding:10px;font-size:11px;font-family:var(--font-mono);word-break:break-all">${link}</textarea><div style="margin-top:12px;text-align:right"><button class="ui-btn" onclick="closeModal()">${t('btn_close')}</button></div></div>`); }
  });
}
...
function openAdviceModal(id){
  ...
  const hist=(r.history||[]).slice().reverse().map(h=>`
    <div style="display:flex;gap:8px;padding:4px 0;font-size:11px;border-bottom:1px solid rgba(29,31,32,.05)">
      ...
      <span style="min-width:0;flex:1"><strong>${adviceStageLabel(h.to)}</strong>${h.by?` <span style="color:var(--color-neutral-600)">${t('advice_hist_by',{by:esc(h.by)})}</span>`:''}
  ...
  const notes=(r.notes||[]).slice().reverse().map(n=>`
    <div style="border:1px solid var(--color-divider);border-radius:5px;background:var(--color-bg);padding:8px 10px;margin-bottom:6px">
      ...
      <div style="font-size:10px;color:var(--color-neutral-500);font-family:var(--font-mono);margin-top:2px">${t('advice_note_meta',{by:esc(n.by),at:fmtDT(n.at)})}</div>
    </div>`).join('')||`<div style="font-size:11px;color:var(--color-neutral-500)">${t('advice_no_notes')}</div>`;
    ...
    <div style="padding:22px 24px">
      <div style="display:flex;align-items:center;gap:9px;margin-bottom:2px">
        ...
        <div style="min-width:0">
          <h2 style="font-family:var(--font-heading);font-weight:600;font-size:17px;margin:0;line-height:1.25">${esc(adviceServiceName(svc))}</h2>
          <div style="font-family:var(--font-mono);font-size:11px;color:var(--color-neutral-600)">${t('advice_submitted_at',{id:r.id,date:fmtDT(r.submittedAt)})}</div>
        </div>
        <span style="margin-left:auto;flex:none">${adviceStageChip(r.status)}</span>
      </div>
      <div style="margin-top:12px">
        ${row(t('advice_row_customer'), r.company?t('advice_party',{name:esc(r.name),company:esc(r.company)}):esc(r.name))}
        ${row(t('advice_row_email'), esc(r.email||t('value_none')))}
        ${r.contractName?row(t('advice_row_contract'), esc(r.contractName)):''}
        ${row(t('advice_row_urgency'), r.urgency==='priority'?t('advice_urgency_priority'):t('advice_urgency_standard'))}
        ${row(t('advice_row_rate'), q.rate?t('advice_rate_per_hr',{amount:fmtKES(q.rate)}):t('value_none'))}
        ${row(t('advice_row_estimate'), q.rate?t('advice_estimate',{min:q.hoursMin,max:q.hoursMax,feeMin:fmtKESshort(q.rate*q.hoursMin),feeMax:fmtKESshort(q.rate*q.hoursMax)}):t('value_none'))}
        ${row(t('advice_row_feedback_due'), `<span style="font-family:var(--font-mono)">${fmtDay(r.eta)}</span>`)}
      </div>
      ${r.description&&r.description!==ADVICE_SEED_DESC?`<div style="margin-top:10px;border:1px solid var(--color-divider);border-radius:5px;background:var(--color-bg);padding:9px 11px;font-size:12px;line-height:1.55;white-space:pre-wrap">${esc(r.description)}</div>`:''}
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:14px">
        <div>
          <h6 style="margin:0 0 6px;font-size:10px;color:var(--color-neutral-600);letter-spacing:.08em;text-transform:uppercase">${t('advice_h_history')}</h6>
          <div class="scroll-thin" style="max-height:150px;overflow-y:auto">${hist}</div>
        </div>
        <div>
          <h6 style="margin:0 0 6px;font-size:10px;color:var(--color-neutral-600);letter-spacing:.08em;text-transform:uppercase">${t('advice_h_notes')}</h6>
          <div class="scroll-thin" style="max-height:150px;overflow-y:auto">${notes}</div>
        </div>
      </div>
      ${canEdit()?`
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:14px">
        <label style="display:block"><span style="display:block;font-size:10.5px;font-weight:600;color:var(--color-neutral-700);margin-bottom:4px;font-family:var(--font-mono)">${t('advice_lbl_assignee')}</span>
          <select id="adv-assignee" style="${selStyle}"><option value="">${t('advice_opt_unassigned')}</option>
            ${members.map(u=>`<option value="${esc(u.name)}" ${r.assignee===u.name?'selected':''}>${t('advice_member_opt',{name:esc(u.name),role:t('role_'+u.role)})}</option>`).join('')}</select></label>
        <label style="display:block"><span style="display:block;font-size:10.5px;font-weight:600;color:var(--color-neutral-700);margin-bottom:4px;font-family:var(--font-mono)">${t('advice_lbl_stage')}</span>
          <select id="adv-status" style="${selStyle}">${ADVICE_STAGES.map(s=>`<option value="${s.k}" ${r.status===s.k?'selected':''}>${t(s.lk)}</option>`).join('')}</select></label>
      </div>
      <label style="display:block;margin-top:10px"><span style="display:block;font-size:10.5px;font-weight:600;color:var(--color-neutral-700);margin-bottom:4px;font-family:var(--font-mono)">${t('advice_lbl_note')}</span>
        <textarea id="adv-note" rows="2" placeholder="${t('advice_note_ph')}" style="${selStyle}"></textarea></label>`:''}
      <div style="margin-top:14px;display:flex;align-items:center;gap:8px">
        <button id="adv-copy-track" class="ui-btn">${icon('copy','w-3.5 h-3.5')} ${t('advice_btn_track')}</button>
        <span style="flex:1"></span>
        <button class="ui-btn" onclick="closeModal()">${t('btn_close')}</button>
        ${canEdit()?`<button id="adv-save" class="ui-btn ui-btn-primary">${t('btn_save')}</button>`:''}
      </div>
    </div>`,{maxWidth:'40rem'});
  document.getElementById('adv-copy-track').addEventListener('click',async()=>{
    try{ await navigator.clipboard.writeText(adviceTrackLink(r)); toast(t('advice_toast_track_copied',{name:r.name})); }catch(e){ toast(t('advice_toast_copy_failed'),'err'); }
  });
  document.getElementById('adv-save')?.addEventListener('click',async()=>{
    ...
    if(patch.status==='Closed' && !await confirmDialog({title:t('advice_dlg_close_title',{id:r.id}), message:t('advice_dlg_close_msg'), confirmLabel:t('advice_dlg_close_confirm'), danger:true})) return;
    try{ await updateAdviceRequest(r.id, patch); toast(t('advice_toast_updated',{id:r.id})); }
    catch(e){ toast(e.message,'err'); }
...
function openRateCardModal(){
  ...
  const rows=Object.values(ADVICE_SERVICES).map(s=>{
    ...
    return `<tr style="border-bottom:1px solid var(--color-divider)">
      <td style="padding:8px 10px 8px 0;min-width:0"><span style="display:flex;align-items:center;gap:7px"><span style="display:inline-flex;color:var(--color-accent-700)">${icon(s.ic,'w-3.5 h-3.5')}</span><span style="font-size:12px;font-weight:500">${esc(adviceServiceName(s))}</span></span></td>
  ...
  openModal(`
    <div style="padding:22px 24px">
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px"><span style="display:inline-flex;color:var(--color-accent)">${icon('coins')}</span>
        <h2 style="font-family:var(--font-heading);font-weight:600;font-size:18px;margin:0">${t('advice_rc_title')}</h2></div>
      <p style="font-size:12px;color:var(--color-neutral-700);margin:0 0 12px;line-height:1.55">${t('advice_rc_intro')} ${editable?t('advice_rc_editable'):t('advice_rc_readonly')}</p>
      <table style="width:100%;border-collapse:collapse">
        <thead><tr style="text-align:left;border-bottom:1px solid var(--color-divider);color:var(--color-neutral-600);font-size:9.5px;letter-spacing:.08em;text-transform:uppercase">
          <th style="padding:6px 10px 6px 0;font-weight:600">${t('advice_rc_th_service')}</th><th style="padding:6px;font-weight:600">${t('advice_rc_th_rate')}</th><th style="padding:6px;font-weight:600">${t('advice_rc_th_min')}</th><th style="padding:6px;font-weight:600">${t('advice_rc_th_max')}</th><th style="padding:6px 0 6px 6px;font-weight:600">${t('advice_rc_th_days')}</th>
        </tr></thead>
        <tbody>${rows}</tbody>
      </table>
      <p style="font-size:10.5px;color:var(--color-neutral-500);margin:10px 0 0;line-height:1.5">${t('advice_rc_note')}</p>
      <div style="margin-top:14px;display:flex;justify-content:flex-end;gap:8px">
        <button class="ui-btn" onclick="closeModal()">${editable?t('dlg_cancel'):t('btn_close')}</button>
        ${editable?`<button id="rt-save" class="ui-btn ui-btn-primary">${t('advice_rc_publish')}</button>`:''}
      </div>
    </div>`,{maxWidth:'36rem'});
  document.getElementById('rt-save')?.addEventListener('click',async()=>{
    ...
    for(const s of Object.values(ADVICE_SERVICES)){
      ...
      if(Object.values(o).some(v=>!Number.isFinite(v)||v<1)){ toast(t('advice_toast_rate_positive'),'err'); return; }
      if(o.hoursMax<o.hoursMin){ toast(t('advice_toast_rate_minmax',{service:adviceServiceName(s)}),'err'); return; }
      out[s.id]=o;
    }
    state.settings.adviceRates=out;
    await saveSettings();
    closeModal(); toast(t('advice_toast_rate_published'));
...
function openAdviceIntakeModal(){
  ...
  openModal(`
    <div style="padding:22px 24px">
      <h2 style="font-family:var(--font-heading);font-weight:600;font-size:18px;margin:0 0 4px">${t('advice_in_title')}</h2>
      <p style="font-size:12px;color:var(--color-neutral-700);margin:0 0 14px;line-height:1.5">${t('advice_in_intro')}</p>
      <label style="display:block;margin-bottom:10px"><span style="display:block;font-size:10.5px;font-weight:600;color:var(--color-neutral-700);margin-bottom:4px;font-family:var(--font-mono)">${t('advice_rc_th_service')}</span>
        <select id="ai-service" style="${inputStyle}">${Object.values(ADVICE_SERVICES).map(s=>{const r=adviceRateFor(s.id);return `<option value="${s.id}">${t('advice_in_service_opt',{name:esc(adviceServiceName(s)),rate:fmtKES(r.rate)})}</option>`;}).join('')}</select></label>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        ${field('ai-name',t('advice_in_name'),t('advice_in_name_ph'))}
        ${field('ai-email',t('advice_in_email'),t('advice_in_email_ph'),'email')}
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        ${field('ai-company',t('advice_in_company'),t('advice_in_company_ph'))}
        ${field('ai-contract',t('advice_in_contract'),t('advice_in_contract_ph'))}
      </div>
      <label style="display:block;margin-bottom:10px"><span style="display:block;font-size:10.5px;font-weight:600;color:var(--color-neutral-700);margin-bottom:4px;font-family:var(--font-mono)">${t('advice_in_need')}</span>
        <textarea id="ai-desc" rows="3" style="${inputStyle}"></textarea></label>
      <label style="display:flex;align-items:center;gap:8px;font-size:12px;color:var(--color-neutral-700);margin-bottom:4px"><input id="ai-priority" type="checkbox" style="width:15px;height:15px;accent-color:var(--color-accent)"/> ${t('advice_urgency_priority')}</label>
      <div style="margin-top:12px;display:flex;justify-content:flex-end;gap:8px">
        <button class="ui-btn" onclick="closeModal()">${t('dlg_cancel')}</button>
        <button id="ai-go" class="ui-btn ui-btn-primary">${t('advice_in_create')}</button>
      </div>
    </div>`,{maxWidth:'34rem'});
  document.getElementById('ai-go').addEventListener('click',async()=>{
    ...
    if(!p.name||!p.email||!p.description){ toast(t('advice_toast_in_required'),'err'); return; }
    try{
      const r=await createAdviceRequest(p);
      closeModal(); renderAdviceDesk(); updateSidebarCounts();
      toast(t('advice_toast_created',{id:r.id,date:fmtDay(r.eta)}));
    }catch(e){ toast(e.message,'err'); }
  });
}
