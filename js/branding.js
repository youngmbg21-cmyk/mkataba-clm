...
function docDesignHeaderHtml(b, c, opts = {}) {
  ...
  if (d.id === 'bold-corporate') {
    ...
    const margin = bleedX || bleedY ? `margin:-${bleedY}px -${bleedX}px 24px;` : 'margin:0 0 24px;border-radius:var(--radius);';
    const pad = bleedX || bleedY ? `padding:18px ${bleedX}px 16px;` : 'padding:18px 22px 16px;';
    const chip = logoTop ? `<span style="display:inline-block;background:#fff;border-radius:var(--radius);padding:5px 8px;line-height:0">${logoTop}</span>` : '';
  ...
  if (d.id === 'compact-executive') {
    ...
    const factsBox = facts.length ? `<div style="display:grid;grid-template-columns:repeat(${Math.min(facts.length, 4)},1fr);gap:8px;background:#eef1f0;border-radius:var(--radius);padding:10px 14px;margin-top:12px">
  ...
  if (d.id === 'modern-editorial') {
    ...
    const tag = `<span style="font-size:10px;letter-spacing:.22em;color:${brAccent(b)};text-transform:uppercase;border:1px solid ${BR_SOFT}40;border-radius:var(--radius);padding:4px 10px;white-space:nowrap;align-self:flex-start">Commercial Agreement</span>`;
