...
function adviceCard(r){
  ...
  return `
    <div data-adv-card="${r.id}" ${drag?'draggable="true"':''} class="q-card" style="background:var(--color-surface);border:1px solid var(--color-divider);border-radius:var(--radius);box-shadow:var(--shadow-sm);padding:11px 12px;cursor:${drag?'grab':'pointer'};display:flex;flex-direction:column;gap:5px">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:6px">
        ...
        <span style="display:flex;align-items:center;gap:4px;flex:none">
          ${r.urgency==='priority'?`<span style="background:var(--st-amber-bg);color:var(--st-amber-fg);font-size:12px;font-weight:600;letter-spacing:.03em;padding:2px 8px;border-radius:var(--radius)">${i18t('adv_priority')}</span>`:''}
          <span style="font-size:12px;font-weight:600;letter-spacing:.03em;padding:2px 8px;border-radius:var(--radius);font-variant-numeric:tabular-nums;background:color-mix(in srgb,${etaCol} 12%,#fff);color:${etaCol}">${etaTxt}</span>
        </span>
      </div>
...
function renderAdviceDesk(){
  ...
  const kpi=(label,val,col)=>`<div style="background:var(--color-surface);border:1px solid var(--color-divider);border-radius:var(--radius);padding:8px 14px;min-width:0">
  ...
  const columnsHtml=groups.map(g=>`
    <div style="min-width:0;display:flex;flex-direction:column;min-height:0">
      <div style="display:flex;align-items:center;gap:6px;padding:0 2px 8px;min-width:0;flex:none">
        ...
        <span style="font-size:12px;background:color-mix(in srgb,var(--color-accent) 11%,transparent);padding:1px 8px;border-radius:var(--radius);color:var(--color-neutral-700);flex:none;font-variant-numeric:tabular-nums">${g.list.length}</span>
      </div>
      <div data-adv-drop="${g.col.k}" class="pipe-col scroll-thin" style="background:color-mix(in srgb,var(--color-accent) 6%,transparent);border:1px solid var(--color-divider);border-radius:var(--radius);padding:8px;display:flex;flex-direction:column;gap:8px;flex:1;min-height:0;overflow-y:auto">
        ${g.list.map(adviceCard).join('')||`<div style="border:1px dashed var(--color-divider);border-radius:var(--radius);padding:22px 10px;text-align:center;font-size:12px;color:var(--color-neutral-500)">${i18t('adv_nothing_here')}</div>`}
      </div>
    </div>`).join('');
...
function wireAdviceBoard(){
  ...
  document.getElementById('adv-link')?.addEventListener('click',async()=>{
    ...
    catch(e){ openModal(`<div style="padding:22px 24px"><h2 style="font-family:var(--font-heading);font-weight:600;font-size:17px;margin:0 0 8px">${i18t('adv_public_intake')}</h2><textarea readonly rows="2" style="width:100%;border:1px solid var(--color-divider);background:var(--color-bg);border-radius:var(--radius);padding:10px;font-size:12px;font-family:var(--font-mono);word-break:break-all">${link}</textarea><div style="margin-top:12px;text-align:right"><button class="ui-btn" onclick="closeModal()">${i18t('act_close')}</button></div></div>`); }
  });
}
...
function openAdviceModal(id){
  ...
  const notes=(r.notes||[]).slice().reverse().map(n=>`
    <div style="border:1px solid var(--color-divider);border-radius:var(--radius);background:var(--color-bg);padding:8px 10px;margin-bottom:6px">
  ...
  const selStyle='width:100%;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:var(--radius);padding:6px 9px;font:inherit;font-size:13px;color:inherit;outline:none';
  openModal(`
    <div style="padding:22px 24px">
      <div style="display:flex;align-items:center;gap:9px;margin-bottom:2px">
        <span style="width:32px;height:32px;flex:none;display:grid;place-items:center;border-radius:var(--radius);background:var(--color-accent-100);color:var(--color-accent-800)">${icon(svc.ic,'w-4 h-4')}</span>
      ...
      ${r.description&&r.description!=='Seeded as sample data'?`<div style="margin-top:10px;border:1px solid var(--color-divider);border-radius:var(--radius);background:var(--color-bg);padding:9px 11px;font-size:13px;line-height:1.55;white-space:pre-wrap">${esc(r.description)}</div>`:''}
...
function openRateCardModal(){
  ...
  const inp=(id,v)=>`<input id="${id}" type="number" min="1" value="${v}" ${editable?'':'disabled'} style="width:100%;border:1px solid var(--color-divider);background:${editable?'var(--color-surface)':'var(--color-bg)'};border-radius:var(--radius);padding:5px 7px;font-family:var(--font-mono);font-size:13px;color:inherit;outline:none"/>`;
...
function openAdviceIntakeModal(){
  const inputStyle='width:100%;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:var(--radius);padding:7px 10px;font:inherit;font-size:14px;color:inherit;outline:none';
