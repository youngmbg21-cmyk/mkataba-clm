  ...
  <div style="min-height:100vh;background:var(--color-bg);">
    <header style="background:var(--color-accent-900);color:#fff;padding:14px 24px;">
      <div style="max-width:1100px;margin:0 auto;display:flex;align-items:center;gap:12px;">
        <div style="width:34px;height:34px;background:var(--color-accent);color:#fff;display:grid;place-items:center;font-family:var(--font-mono);font-weight:600;font-size:15px;letter-spacing:.02em;border-radius:var(--radius);flex:none;">HT</div>
        <div style="line-height:1.25;min-width:0;">
          ...
          <div style="font-size:12px;color:var(--color-accent-200);font-family:var(--font-mono);">${i18t('apo_tagline')}</div>
        </div>
      </div>
    </header>
    <div style="max-width:1100px;margin:0 auto;padding:28px 24px;">${inner}</div>
    <p style="max-width:1100px;margin:0 auto;padding:0 24px 28px;font-size:12px;color:var(--color-neutral-500);line-height:1.6">
...
function renderAdviceIntake(){
  ...
  const inputStyle='width:100%;min-height:36px;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:var(--radius);padding:7px 11px;font-size:14px;font-family:var(--font-body);color:var(--color-text);outline:none';
  const field=(id,label,ph,type='text')=>`<label style="display:block;margin-bottom:10px"><span style="display:block;font-size:12px;font-weight:600;color:var(--color-neutral-700);margin-bottom:4px;font-family:var(--font-mono);letter-spacing:.02em">${label}</span><input id="${id}" type="${type}" placeholder="${ph}" style="${inputStyle}"/></label>`;
  const svcCard=s=>{
    const r=adviceRateFor(s.id);
    return `
    <label data-svc="${s.id}" style="display:block;border:1.5px solid var(--color-divider);border-radius:var(--radius);background:var(--color-surface);padding:13px 14px;cursor:pointer;transition:border-color .12s ease, box-shadow .12s ease">
      <input type="radio" name="adv-svc" value="${s.id}" style="position:absolute;opacity:0"/>
      <div style="display:flex;align-items:center;gap:8px">
        <span style="width:30px;height:30px;flex:none;display:grid;place-items:center;border-radius:var(--radius);background:var(--color-accent-100);color:var(--color-accent-800)">${icon(s.ic,'w-4 h-4')}</span>
      ...
      <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;font-size:12px">
        <span style="background:var(--color-bg);border:1px solid var(--color-divider);border-radius:var(--radius);padding:2px 8px;color:var(--color-neutral-700);font-variant-numeric:tabular-nums">typically ${r.hoursMin}–${r.hoursMax} hrs · ${fmtMoneyShort(r.rate*r.hoursMin)}–${fmtMoneyShort(r.rate*r.hoursMax)}</span>
        <span style="background:var(--st-green-bg);border:1px solid var(--st-green-line);border-radius:var(--radius);padding:2px 8px;color:var(--st-green-fg);font-variant-numeric:tabular-nums">feedback in ~${r.days} business day${r.days===1?'':'s'}</span>
      </div>
    </label>`;
  };
  root.innerHTML=advicePortalShell(`
    <div style="display:grid;gap:22px;align-items:start" class="portal-grid">
      ...
      <aside style="background:var(--color-surface);border:1px solid var(--color-divider);border-radius:var(--radius);box-shadow:var(--shadow-sm);padding:18px" class="portal-aside">
        <h2 style="font-family:var(--font-heading);font-weight:600;font-size:16px;color:var(--color-text);margin:0 0 4px">${i18t('apo_submit_request')}</h2>
        <p style="font-size:12px;color:var(--color-neutral-700);margin:0 0 14px;line-height:1.5">${ADVICE_PORTAL.remote?'Your request goes straight to the legal team — no account needed.':'Demo mode — this request is stored in this browser only.'}</p>
        ...
        <label style="display:block;margin-bottom:12px"><span style="display:block;font-size:12px;font-weight:600;color:var(--color-neutral-700);margin-bottom:4px;font-family:var(--font-mono);letter-spacing:.02em">${i18t('apo_describe')}</span>
          <textarea id="ap-desc" rows="4" placeholder="${esc(i18t('po_ph_advice_example'))}" style="${inputStyle}min-height:0"></textarea></label>
        <label style="display:flex;align-items:flex-start;gap:9px;font-size:13px;color:var(--color-neutral-700);margin-bottom:14px;line-height:1.45"><input id="ap-priority" type="checkbox" style="width:15px;height:15px;accent-color:var(--color-accent);margin-top:1px"/><span><strong>${i18t('apo_priority')}</strong> ${i18t('apo_priority_note')}</span></label>
        <button id="ap-go" class="ui-btn ui-btn-primary" style="width:100%;padding:10px;font-size:14px">${icon('send','w-4 h-4')} Submit request</button>
  ...
  document.getElementById('ap-go').addEventListener('click',async()=>{
    const sid=document.querySelector('input[name="adv-svc"]:checked')?.value;
    if(!sid){ toast(i18t('apo_pick_service_first'),'err'); return; }
    ...
    document.getElementById('ap-result').innerHTML=`
      <div style="border:1px solid color-mix(in srgb,var(--st-green-dot) 30%,transparent);background:var(--st-green-bg);border-radius:var(--radius);padding:14px">
        <div style="display:flex;align-items:center;gap:6px;color:var(--st-green-fg);font-size:14px;font-weight:600;margin-bottom:4px">${icon('check2','w-4 h-4')} Request ${pesc(req.id)} submitted</div>
        <p style="font-size:12px;color:var(--color-neutral-700);margin:0 0 8px;line-height:1.5">${i18t('apo_estimated_by')} <strong>${fmtDay(req.eta)}</strong>${i18t('apo_follow_stage')}</p>
        <textarea id="ap-link" readonly rows="2" style="width:100%;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:var(--radius);padding:9px;font-size:12px;font-family:var(--font-mono);color:var(--color-text);outline:none;word-break:break-all">${link}</textarea>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px">
          <button id="ap-copy" class="ui-btn" style="padding:8px;font-size:13px">${icon('copy','w-3 h-3')} ${i18t('po_copy_link')}</button>
...
function renderAdviceTracking(r){
  ...
  if(!r){
    root.innerHTML=advicePortalShell(`
      <div style="display:grid;place-items:center;padding:40px 0">
        <div style="background:var(--color-surface);border:1px solid var(--color-divider);box-shadow:var(--shadow-lg);border-radius:var(--radius);padding:32px;text-align:center;max-width:24rem">
          <div style="color:var(--st-ruby-dot);margin-bottom:12px;display:flex;justify-content:center">${icon('ban','w-8 h-8')}</div>
          <h1 style="font-family:var(--font-heading);font-weight:600;font-size:20px;color:var(--color-text);margin:0">${i18t('apo_not_found')}</h1>
          <p style="font-size:14px;color:var(--color-neutral-700);margin-top:6px;line-height:1.5">${i18t('apo_invalid_link')}</p>
        </div>
      </div>`);
    return;
  }
  ...
  const row=(k,v)=>`<div style="display:flex;justify-content:space-between;gap:10px;padding:5px 0;border-bottom:1px solid color-mix(in srgb,var(--color-text) 7%,transparent);font-size:13px"><span style="color:var(--color-neutral-600);flex:none">${k}</span><span style="font-weight:400;text-align:right;min-width:0">${v}</span></div>`;
  const etaBanner = closed
    ? `<div style="border:1px solid var(--st-ruby-line);background:var(--st-ruby-bg);border-radius:var(--radius);padding:12px 14px;font-size:13px;color:var(--st-ruby-fg);line-height:1.5">This request was closed without delivery${reached('Closed')?` on ${fmtDay(reached('Closed').at)}`:''}. Contact the legal team if that's unexpected.</div>`
    : r.status==='Delivered'
    ? `<div style="border:1px solid var(--st-green-line);background:var(--st-green-bg);border-radius:var(--radius);padding:12px 14px;font-size:13px;color:var(--st-green-fg);line-height:1.5"><strong>Delivered${reached('Delivered')?' '+fmtDay(reached('Delivered').at):''}.</strong> ${i18t('apo_feedback_with_you')}</div>`
    : `<div style="border:1px solid var(--color-divider);background:var(--color-accent-100);border-radius:var(--radius);padding:12px 14px;font-size:13px;color:var(--color-accent-800);line-height:1.5">${i18t('apo_estimated_by')} <strong>${fmtDay(r.eta)}</strong>${left!=null?(left<0?` — running ${-left} day${-left===1?'':'s'} over, the team is on it`:left===0?' — that’s today':` — ${left} day${left===1?'':'s'} away`):''}.</div>`;
  root.innerHTML=advicePortalShell(`
    <div style="display:grid;gap:22px;align-items:start" class="portal-grid">
      <div style="background:var(--color-surface);border:1px solid var(--color-divider);border-radius:var(--radius);box-shadow:var(--shadow-sm);padding:22px 24px">
        <div style="display:flex;align-items:center;gap:9px;margin-bottom:14px">
          <span style="width:34px;height:34px;flex:none;display:grid;place-items:center;border-radius:var(--radius);background:var(--color-accent-100);color:var(--color-accent-800)">${icon(svc.ic,'w-4 h-4')}</span>
      ...
      <aside style="background:var(--color-surface);border:1px solid var(--color-divider);border-radius:var(--radius);box-shadow:var(--shadow-sm);padding:18px" class="portal-aside">
        <h2 style="font-family:var(--font-heading);font-weight:600;font-size:15px;color:var(--color-text);margin:0 0 8px">${i18t('apo_fee_estimate')}</h2>
        ...
        <p style="font-size:12px;color:var(--color-neutral-500);margin:10px 0 0;line-height:1.55">The final fee is confirmed with you at Scoping before billable work starts — you will never be invoiced beyond what is agreed there.</p>
        <button id="at-new" class="ui-btn" style="width:100%;margin-top:14px;padding:8px;font-size:13px">${icon('plus','w-3.5 h-3.5')} Submit another request</button>
      </aside>
    </div>
