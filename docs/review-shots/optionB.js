...
(async()=>{
  ...
  const m=await page.evaluate(()=>{
    ...
    // COMMERCIAL TERMS goes on the LEFT, under the Key terms card, in the space that is empty today
    const grid=document.querySelector('.terms-grid'); const leftCol=grid.children[0];
    const ct=document.createElement('section'); ct.style.cssText=CARD+';margin-top:16px';
    ct.innerHTML=head('Commercial terms','2 departures','amber')
      ...
      +`<p style="margin:9px 0 0;font-size:var(--t-label);color:var(--color-neutral-600);text-align:left">Read from the wording, not typed. Where it says nothing, this says nothing.</p>`;
    leftCol.appendChild(ct);
    // DOCUMENTS stays on the right, after the brief and before the family
    const side=document.querySelector('#kt-side'); const fam=document.querySelector('#family-section');
  ...
  const after=await page.evaluate(()=>{
    const side=document.querySelector('#kt-side'); const grid=document.querySelector('.terms-grid');
    const L=grid.children[0];
    return { right:[...side.children].filter(e=>e.getBoundingClientRect().height>4).map(e=>(e.querySelector('h6,h4,h3')||{}).textContent),
      rightStack:side.scrollHeight, rightTrack:side.clientHeight, rightScrolls:side.scrollHeight>side.clientHeight+2,
      leftStack:[...L.children].reduce((n,e)=>n+e.getBoundingClientRect().height,0)|0, leftTrack:Math.round(L.getBoundingClientRect().height) };
  });
  console.log('OPTION B', JSON.stringify(after));
  await page.screenshot({path:path.join(OUT,'C4-room-option-b.png')});
  fs.writeFileSync(OUT+'/optionB.json',JSON.stringify(after,null,1));
