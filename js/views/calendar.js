...
const CAL_EVENT = {
  expiry:     { dot:'var(--st-ruby-dot)',  fg:'var(--st-ruby-fg)',  get label(){ return i18t('cal_expiry'); },           tint:'rgba(244,63,94,.13)' },
  renewal:    { dot:'var(--st-amber-dot)', fg:'var(--st-amber-fg)', get label(){ return i18t('cal_renewal_decision'); }, tint:'rgba(245,158,11,.14)' },
  obligation: { dot:'var(--st-green-dot)', fg:'var(--st-green-fg)', get label(){ return i18t('cal_obligation'); },       tint:'rgba(16,185,129,.13)' },
};
  ...
  <div class="view-enter" style="height:var(--view-h);box-sizing:border-box;padding:14px 16px 18px;display:flex;flex-direction:column;gap:14px">
    ...
    <div id="cal-grid" style="flex:1;min-height:0;overflow-y:auto;display:grid;grid-template-columns:repeat(7,minmax(0,1fr));grid-template-rows:repeat(6,1fr);gap:4px">${cells.join('')}</div>
      ...
      <section class="cal-card">
        <h4 style="font-family:var(--font-heading);font-size:15px;font-weight:700;margin:0 0 10px;color:var(--color-text);flex:none">${i18t('cal_next_60')}</h4>
