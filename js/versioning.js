...
const _diffLegend = `<span style="display:inline-flex;align-items:center;gap:var(--s-2)"><ins style="background:var(--st-green-bg);color:var(--st-green-fg);text-decoration:none;border-radius:var(--radius);padding:0 var(--s-1)">added</ins><del style="background:var(--st-ruby-bg);color:var(--st-ruby-fg);border-radius:var(--radius);padding:0 var(--s-1)">removed</del></span>`;
const _diffBox = (a,b)=>`<div class="scroll-thin" style="border:1px solid var(--color-divider);border-radius:var(--radius);background:var(--color-surface);padding:14px var(--s-4);font-size:var(--t-body);line-height:1.85;color:var(--color-doc-text);max-height:56vh;overflow-y:auto;white-space:pre-wrap;font-family:var(--font-body)">${diffHtml(a,b)}</div>`;
...
function openCompareModal(c){
  ...
  const selStyle='font:inherit;font-size:var(--t-body);border:1px solid var(--color-divider);background:var(--color-surface);padding:6px var(--s-2);border-radius:var(--radius);color:inherit;min-width:0;flex:1';
  ...
  const structuredBox=(aText,bText)=>{
    ...
    return `<div class="cmp-doc" style="font-size:var(--t-body);line-height:1.75;border:1px solid var(--color-divider);border-radius:var(--radius);background:var(--color-surface);color:var(--color-doc-text);padding:14px 18px;max-height:62vh;overflow:auto">${html}</div>`;
  };
  const foldUnchanged=()=>{
  ...
  const segBtn=(k,label,on)=>`<button data-cmp-mode="${k}" style="border:0;border-radius:var(--radius);padding:5px var(--s-3);font:inherit;font-size:var(--t-meta);font-weight:var(--w-strong);cursor:pointer;background:${on?'var(--accent-solid,var(--color-accent))':'none'};color:${on?'#fff':'var(--color-neutral-600)'}">${label}</button>`;
  openModal(`
    <div style="padding:20px 22px">
      ...
      <style>
        ...
        #cmp-out .cmp-fold{display:block;width:100%;text-align:center;border:1px dashed var(--color-divider);background:var(--color-neutral-100);color:var(--color-neutral-600);border-radius:var(--radius);font:inherit;font-size:var(--t-label);font-weight:var(--w-strong);padding:var(--s-1);margin:var(--s-2) 0;cursor:pointer}
  ...
  const runCum=()=>{
    ...
    const trail=moved.length
      ? `<div style="font-size:var(--t-meta);line-height:1.9;color:var(--color-neutral-700);border:1px solid var(--color-divider);border-radius:var(--radius);padding:9px var(--s-3);margin-bottom:10px"><b>${i18t('ve_clauses_moved')}</b> ${moved.slice(0,8).map(m=>`${m.label} ×${m.n}`).join(' · ')}${moved.length>8?` · +${moved.length-8} more`:''}</div>`
      : '';
