...
function emailSetupBannerHtml(){
  if(!emailOff()) return '';
  return `
    <div id="email-setup-banner" style="display:flex;align-items:flex-start;flex-wrap:wrap;gap:11px;border:1px solid var(--st-amber-line);background:var(--st-amber-bg);border-left:4px solid var(--st-amber-dot);border-radius:var(--radius);padding:12px 16px">
...
/* ---- ONE READING, THREE DRESSES (24 Aug 2026) ----
   The register's row now states its stage as a coloured DOT beside a coloured
   WORD rather than as a filled chip, which makes three dresses for one reading:
   the chip (cards, lists, panels), the head's sentence, and the table's dot.
   Three copies of "which branch applies" is how they come to disagree — this
...
const questionDot = cid => { const n=questionCount(cid); if(!n) return '';
  return `<span title="${i18tn('co_questions_waiting',n,{n})}" style="display:inline-block;margin-right:6px;vertical-align:middle;font-size:12px;font-weight:700;font-family:var(--font-mono);background:var(--st-amber-bg);color:var(--st-amber-fg);border-radius:var(--radius);padding:1px 6px;flex:none">${n}&nbsp;?</span>`; };
...
function toast(msg,kind,opts){
  ...
  el.innerHTML='<span style="display:inline-flex;flex:none">'+icon(spec.ic)+'</span>'
    ...
    +(act?'<button type="button" data-toast-act style="flex:none;border:1px solid rgba(255,255,255,.55);'
      +'background:rgba(255,255,255,.14);color:#fff;border-radius:var(--radius);font:inherit;font-size:13px;'
...
function renderAuth(mode){
  ...
  <div style="min-height:100vh;display:grid;place-items:center;background:var(--color-bg);padding:40px 16px;">
    <div style="width:100%;max-width:420px;">
      <div style="display:flex;align-items:center;gap:10px;justify-content:center;margin-bottom:20px;">
        <div style="width:36px;height:36px;background:var(--color-accent-800);color:#fff;display:grid;place-items:center;font-family:var(--font-mono);font-weight:600;font-size:17px;letter-spacing:.02em;border-radius:var(--radius);">HT</div>
  ...
  const input=(id,label,type='text',ph='')=>`
    <label style="display:block;margin-bottom:14px;">
      ...
      <input id="${id}" type="${type}" placeholder="${ph}" style="width:100%;min-height:36px;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:var(--radius);padding:7px 11px;font-size:14px;font-family:var(--font-body);color:var(--color-text);outline:none;"/></label>`;
  ...
  if(mode==='setup'){
    root.innerHTML = shell(`
      ...
      <label style="display:block;margin-bottom:14px;">
        ...
        <select id="su-market" style="width:100%;min-height:36px;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:var(--radius);padding:7px 11px;font-size:14px;font-family:var(--font-body);color:var(--color-text);outline:none;">
    ...
    document.getElementById('fp-go').addEventListener('click',async()=>{
      ...
      try{
        ...
        document.getElementById('fp-result').innerHTML=`<div style="border-radius:var(--radius);background:var(--color-accent-100);border:1px solid var(--color-divider);padding:11px;font-size:12px;color:var(--color-accent-800);line-height:1.5;">${i18t('co_reset_sent')}${outboxHint}</div>`;
...
function renderMustChangePassword(){
  ...
  const F='width:100%;min-height:38px;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:var(--radius);padding:8px 11px;font-size:14px;font-family:var(--font-body);color:var(--color-text);outline:none;margin-bottom:10px;';
  authRoot.innerHTML=`
...
function confirmDialog(opts={}){
  ...
  return new Promise(resolve=>{
    ...
    ov.innerHTML=`
      ...
      <div class="modal-in" role="alertdialog" aria-modal="true" style="position:relative;width:100%;max-width:30rem;background:var(--color-surface);border:1px solid var(--color-divider);box-shadow:var(--shadow-lg);border-radius:var(--radius);padding:22px 24px">
        <div style="display:flex;align-items:flex-start;gap:12px;margin-bottom:${message?'6px':'14px'}">
          <span style="width:34px;height:34px;flex:none;display:grid;place-items:center;border-radius:var(--radius);background:${danger?'var(--red-tint,rgba(176,69,60,.1))':'var(--color-accent-100)'};color:${danger?'var(--danger)':'var(--color-accent-700)'}">${icon(danger?'alert':'shield','w-4 h-4')}</span>
...
function promptDialog(opts={}){
  ...
  return new Promise(resolve=>{
    ...
    ov.innerHTML=`
      <div style="position:absolute;inset:0;background:color-mix(in srgb,#2b2b2d 50%,transparent)"></div>
      <div class="modal-in" role="dialog" aria-modal="true" style="position:relative;width:100%;max-width:30rem;background:var(--color-surface);border:1px solid var(--color-divider);box-shadow:var(--shadow-lg);border-radius:var(--radius);padding:22px 24px">
        <div style="display:flex;align-items:flex-start;gap:12px;margin-bottom:${message?'6px':'12px'}">
          <span style="width:34px;height:34px;flex:none;display:grid;place-items:center;border-radius:var(--radius);background:var(--color-accent-100);color:var(--color-accent-700)">${icon('pencil','w-4 h-4')}</span>
          <h3 style="font-family:var(--font-heading);font-weight:600;font-size:17px;margin:0;line-height:1.3;padding-top:5px">${esc(title)}</h3>
        </div>
          ...
          ${opts.multiline
            ? /* A REASON IS NOT A ONE-LINE ANSWER. box-sizing so the padding
              ...
              `<textarea id="pd-input" rows="3" wrap="soft" placeholder="${esc(placeholder).replace(/"/g,'&quot;')}"
                 style="box-sizing:border-box;width:100%;max-width:100%;min-height:70px;resize:vertical;border:1px solid var(--color-divider);background:var(--color-bg);border-radius:var(--radius);padding:8px 11px;font:inherit;font-size:14px;line-height:1.6;outline:none;white-space:pre-wrap;overflow-wrap:anywhere">${esc(opts.value)}</textarea>`
            : `<input id="pd-input" type="text" value="${esc(opts.value).replace(/"/g,'&quot;')}" placeholder="${esc(placeholder).replace(/"/g,'&quot;')}"
                 style="width:100%;border:1px solid var(--color-divider);background:var(--color-bg);border-radius:var(--radius);padding:8px 11px;font:inherit;font-size:14px;outline:none"/>`}
...
function sharePurposePickerHtml(c, sel){
  const btn=(k)=>{ const on=sel===k, m=SHARE_PURPOSE_COPY[k];
    return `<button type="button" data-share-purpose="${k}" aria-pressed="${on?'true':'false'}"
      style="flex:1;min-width:190px;text-align:left;cursor:pointer;font:inherit;border-radius:var(--radius);padding:10px 12px;
      border:1.5px solid ${on?'var(--color-accent)':'var(--color-divider)'};
      background:${on?'var(--color-accent-100)':'var(--color-surface)'}">
...
function shareSummaryStepHtml(c, opts={}){
  ...
  const rows = s && s.lines.length ? s.lines.map(x=>`
    <li style="display:flex;gap:9px;align-items:flex-start;padding:7px 0;border-bottom:1px solid var(--color-divider)">
      ...
      <span style="flex:none;font-size:11px;font-weight:700;letter-spacing:.09em;text-transform:uppercase;border-radius:var(--radius);padding:2px 6px;margin-top:1px;
  ...
  const FLD='width:100%;min-height:var(--field-h);border:1px solid var(--field-line);background:var(--color-surface);border-radius:var(--radius);padding:var(--field-pad-y) var(--field-pad-x);font-size:var(--field-size);font-family:var(--font-body);color:var(--color-text);line-height:var(--field-lh);';
  ...
  return `
    <div id="share-step-1"${opts.hiddenStart?' class="hidden"':''}>
      ...
      <div id="share-hist-note"${hist?'':' class="hidden"'} style="margin:0 0 14px;border:1px solid var(--color-accent-300);background:var(--color-accent-100);border-radius:var(--radius);padding:10px 12px;font-size:13px;line-height:1.55;color:var(--color-accent-800)">
      ...
      <div id="share-manifest"${hist?' class="hidden"':''}>
        ${s&&s.lines.length?`<ul style="list-style:none;margin:0 0 14px;padding:0;max-height:230px;overflow-y:auto;border:1px solid var(--color-divider);border-radius:var(--radius);padding:2px 11px">${rows}</ul>`
          :`<div style="margin:0 0 14px;border:1px dashed var(--color-divider);border-radius:var(--radius);padding:14px;font-size:13px;color:var(--color-neutral-600);text-align:center">${i18t('co_nothing_proposed')}</div>`}
      </div>
      ...
      ${opts.handOver?`<div id="share-handover" style="margin-top:12px;border:1px solid var(--st-green-line);background:var(--st-green-bg);border-left:3px solid var(--st-green-fg);border-radius:var(--radius);padding:9px 12px;font-size:13px;line-height:1.5;color:var(--st-green-fg)">
...
function shareSignerPickHtml(c, sel){
  ...
  return `<div id="share-signers-box" data-need-signers="${need?'1':'0'}" style="margin:0 0 14px;${box};border-radius:var(--radius);padding:11px 12px">
...
function readinessPanelHtml(c){
  ...
  return `<div id="share-readiness" style="margin:0 0 12px;border:1px solid ${tone.line};background:${tone.bg};border-radius:var(--radius);padding:10px 12px;">
...
function quickSendStepHtml(c, pre, purpose, warns){
  ...
  <div id="share-step-0">
    ...
    <div style="border:1px solid var(--color-divider);background:var(--color-bg);border-radius:var(--radius);padding:14px 16px;margin-bottom:10px">
...
async function openShareModal(c, opts={}){
  ...
  const tab=(k,label,active)=>`<button data-share-ch="${k}" style="flex:1;padding:7px 4px;font:inherit;font-size:13px;font-weight:600;cursor:pointer;border:1px solid ${active?'var(--color-accent)':'var(--color-divider)'};background:${active?'var(--color-accent)':'var(--color-surface)'};color:${active?'#fff':'var(--color-neutral-700)'};border-radius:var(--radius)">${label}</button>`;
  ...
  shareFillModal(`
    <div style="padding:22px 24px;">
      ...
      ${emailOff()?`<div id="sh-noemail" style="margin:0 0 12px;border:1px solid var(--st-amber-line);background:var(--st-amber-bg);border-radius:var(--radius);padding:10px 12px;font-size:13px;line-height:1.55;color:var(--st-amber-fg)">
      ...
      ${server?'':`<div style="margin:0 0 12px;border:1px solid var(--st-ruby-line);background:var(--st-ruby-bg);border-radius:var(--radius);padding:10px 12px;">
      ...
      <div id="share-fields">
        ${preNote?`<div id="sh-prefill-note" data-prefill-src="${attr(pre.source)}" style="display:flex;align-items:center;gap:7px;margin:0 0 9px;font-size:13px;color:var(--color-neutral-700);border:1px solid var(--color-divider);background:var(--color-bg);border-radius:var(--radius);padding:7px 10px">
        ...
        <label style="display:flex;align-items:center;gap:8px;margin-top:10px;font-size:13px;color:var(--color-neutral-700)">Link expires in
          <select id="sh-exp" style="border:1px solid var(--color-divider);background:var(--color-surface);border-radius:var(--radius);padding:4px 6px;font:inherit;font-size:13px;color:inherit;">
  ...
  const copyBox=(link,note)=>`
    <div style="border:1px solid var(--color-divider);background:var(--color-accent-100);border-radius:var(--radius);padding:12px;">
      ...
      <textarea id="share-link" readonly rows="3" style="width:100%;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:var(--radius);padding:9px;font-size:12px;font-family:var(--font-mono);color:var(--color-text);outline:none;word-break:break-all;">${link}</textarea>
  ...
  const doSend=async()=>{
    ...
    if(server){
      ...
      reuseNote = reuse ? `<div style="border:1px solid var(--color-divider);background:var(--color-accent-100);border-radius:var(--radius);padding:10px 12px;font-size:13px;line-height:1.55;color:var(--color-accent-800);margin-bottom:8px">
      ...
      if(ch==='email'){
        ...
        if(r.emailSent){
          resultBox(`<div style="border:1px solid color-mix(in srgb,var(--st-green-dot) 30%,transparent);background:var(--st-green-bg);border-radius:var(--radius);padding:12px;font-size:13px;color:var(--st-green-fg);display:flex;align-items:flex-start;gap:8px;">${icon('check2','w-4 h-4')}<span><strong>${i18t('co_email_sent')}</strong> to ${esc(email)}. Their answers land on the contract here as they make them — watch it in Negotiation rather than your inbox. Fill in another recipient to share again.</span></div>`);
          ...
          resultBox(`<div style="border:1px solid color-mix(in srgb,var(--st-green-dot) 30%,transparent);background:var(--st-green-bg);border-radius:var(--radius);padding:12px;font-size:13px;color:var(--st-green-fg);display:flex;align-items:flex-start;gap:8px;">${icon('check2','w-4 h-4')}<span><strong>${i18t('co_already_sent')}</strong> ${esc(email)} received their link ${fmtDT(r.alreadySentAt)}, and it is still live — no duplicate was sent. This link now shows the current version.${link}</span></div>`);
          ...
          resultBox(`<div style="border:1px solid var(--st-steel-line);background:var(--st-steel-bg);border-radius:var(--radius);padding:12px;font-size:13px;color:var(--st-steel-fg);display:flex;align-items:flex-start;gap:8px;">${icon('clock','w-4 h-4')}<span><strong>${i18t('co_link_held')}</strong> ${
          ...
          resultBox(`<div style="border:1px solid color-mix(in srgb,var(--st-amber-dot) 45%,transparent);background:color-mix(in srgb,var(--st-amber-dot) 10%,transparent);border-radius:var(--radius);padding:12px;font-size:13px;color:var(--st-amber-fg);display:flex;align-items:flex-start;gap:8px;">${icon('alert','w-4 h-4')}<span><strong>${i18t('co_not_delivered')}</strong> The link was created and is safe to send another way, but ${esc(email)} has not received anything.${r.emailError?`<br><span style="display:inline-block;margin-top:6px;font-family:var(--font-mono);font-size:12px;line-height:1.5">${esc(r.emailError)}</span>`:''}${link}</span></div>`);
        } else {
          resultBox(`<div style="border:1px solid color-mix(in srgb,var(--st-amber-dot) 45%,transparent);background:color-mix(in srgb,var(--st-amber-dot) 10%,transparent);border-radius:var(--radius);padding:12px;font-size:13px;color:var(--st-amber-fg);display:flex;align-items:flex-start;gap:8px;">${icon('alert','w-4 h-4')}<span><strong>${i18t('co_queued_not_sent')}</strong> This server has no mail provider set up, so nothing left HaTi. An admin can read the message and the link in the outbox under Team &amp; Settings.${link}</span></div>`);
        }
      } else if(ch==='whatsapp'){
...
function reshareNotSentModal(c, out, who){
  ...
  openModal(`
    <div style="padding:22px 24px">
      ...
      <textarea id="rs-link" readonly rows="3" style="width:100%;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:var(--radius);padding:9px;font-size:12px;font-family:var(--font-mono);color:var(--color-text);outline:none;word-break:break-all">${esc(out.link||'')}</textarea>
...
function counterpartySeenHtml(c, shares){
  ...
  if(st.kind==='opened') return `
    <div id="seen-state" style="display:flex;align-items:center;gap:9px;flex-wrap:wrap;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:var(--radius);padding:9px 13px;margin-bottom:12px;font-size:13px;color:var(--color-neutral-700)">
  ...
  return `
    <div id="seen-state" style="display:flex;align-items:center;gap:9px;flex-wrap:wrap;border:1px solid ${stale?'var(--st-amber-line)':'var(--color-divider)'};background:${stale?'var(--st-amber-bg)':'var(--color-surface)'};border-radius:var(--radius);padding:9px 13px;margin-bottom:12px;font-size:13px;color:${stale?'var(--st-amber-fg)':'var(--color-neutral-700)'}">
...
function shareJourneyHtml(c, shares){
  ...
  return `<div id="share-journey" style="border:1px solid ${j.stale?'var(--st-amber-line)':'var(--color-divider)'};background:${j.stale?'var(--st-amber-bg)':'var(--color-bg)'};border-radius:var(--radius);padding:11px 13px;margin-bottom:12px">
  ...
  host.innerHTML=`<div class="px-5 py-4">
    ...
    <div class="space-y-2">
      ${shares.map(s=>{
        ...
        return `<div style="border:1px solid var(--color-divider);border-radius:var(--radius);padding:8px 10px;background:var(--color-bg)">
...
function openImportModal(c){
  openModal(`
    <div style="padding:22px 24px;">
      ...
      <textarea id="imp-code" rows="5" placeholder="${i18t('co_paste_response')}" style="width:100%;border:1px solid var(--color-divider);background:var(--color-bg);border-radius:var(--radius);padding:11px;font-size:12px;font-family:var(--font-mono);color:var(--color-text);outline:none;"></textarea>
      <div style="margin-top:14px;display:flex;align-items:center;gap:8px;justify-content:flex-end;">
