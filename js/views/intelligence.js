...
async function intelComplianceScan(q){
  ...
  if(!rows.length){
    intel.history.push({role:'assistant', text:`Good news — a first-pass review of your ${cs.length} live contract${cs.length===1?'':'s'} surfaced no clauses flagged as potentially risky, unlawful or missing. Open any contract and run <b>${t('int_copilot_review')}</b> for a deeper per-contract check.`});
    return;
  }
  ...
  let html=`<b>${t('int_compliance_review')}</b> ${rows.length} of ${cs.length} live contracts carry clauses worth a closer look — ${totalFindings} potential issue${totalFindings===1?'':'s'} in all (risks, missing protections or ambiguous terms), ranked by severity. This is a first-pass review to raise with counsel, not legal advice.`;
  html+=top.map(r=>{
  ...
  if(rows.length>top.length) html+=`<div style="font-size:11px;color:var(--color-neutral-600);margin-top:9px">…and ${rows.length-top.length} more flagged contract${rows.length-top.length===1?'':'s'}. Ask me about any one by name or id for the detail, or open it and run <b>${t('int_copilot_review')}</b>.</div>`;
...
async function intelRunCompare(){
  ...
  if(ids.length<2){ if(typeof toast==='function') toast(t('int_stage_two'),'err'); return; }
  ...
  try{
    if(typeof copilotAvailable==='function' && copilotAvailable()){
      ...
      localFallback('Side-by-side from your live contract data. <span class="text-[11px] text-amber-700">' + t('int_add_key') + '</span>');
    }
...
function updateIntelNote(){
  ...
  el.innerHTML = intel.busy ? `<span class="text-brand-700">${t('int_thinking')}</span>`
    : `<span class="text-ink/60">${t('int_grouped_by')} <b class="text-ink">${gb}</b>${on.length?` · <b class="text-brand-700">${on.map(l=>igEsc(l.label)).join(' ∩ ')}</b> <span class="text-ink/40">· ${act.ids?act.ids.size:0} ${act.action==='filter'?'shown':'highlighted'}</span>`:''}</span>`
      + ((on.length||intel.groups)?` <button id="ig-clear" class="ml-2 text-[11px] font-600 text-brand-600 hover:text-brand-800">${t('int_clear_all_x')}</button>`:'');
...
function renderIntelLegend(model){
  ...
  el.innerHTML=`<div class="text-[10px] uppercase tracking-wider text-ink/40 mb-1.5">${t('int_status_click')}</div>`+
  ...
  <div class="view-enter" style="height:var(--view-h);display:flex;flex-direction:column;min-height:0">
    ...
    <div class="relative flex-1 min-h-0 bg-canvas flex" style="flex:1;min-height:0;display:flex;position:relative;background:var(--color-bg)">
      <div class="relative flex-1 min-w-0" style="flex:1;min-width:0;position:relative">
        ...
        <div class="absolute right-4 bottom-4 text-[11px] text-ink/40 bg-white border border-line rounded-lg px-2.5 py-1.5">${t('int_drag_nodes')}</div>
      </div>
...
function intelFrictionHtml(){
  ...
  const clauseHero=top?hero(pct(top.share)+'%','var(--color-text)',
    ...
    <br><button data-igf-standards style="${LINK}">${t('int_open_clause_std')}</button>`):'';
  ...
  const deadHero=hero(String(st.deadlocks), st.deadlocks?'var(--st-amber-fg,#b45309)':'var(--color-text)',
    st.deadlocks
      ?`change${st.deadlocks===1?' is':'s are'} <b>${t('int_refused_open')}</b> — nobody withdrew ${st.deadlocks===1?'it':'them'}, so ${dealCount===1?'one deal is':dealCount+' deals are'} waiting on a decision somebody has to make.
  ...
  const left=`<div style="padding:12px 18px;min-width:0">
    <div style="font-size:16px;font-weight:700;letter-spacing:-.01em">${t('int_what_slowing')}</div>
  ...
  const right=`<div style="padding:12px 18px;border-left:1px solid var(--color-divider);min-width:0">
    <div style="display:flex;align-items:baseline;gap:10px"><span style="font-size:13px;font-weight:700">${t('int_most_contested')}</span>
    ...
    <div role="img" aria-label="${t('int_bar_chart_aria')}" style="display:grid;grid-template-columns:minmax(120px,170px) 1fr 40px 40px;gap:5px 9px;align-items:center;margin:8px 0 12px">${bars}</div>
    <div style="display:flex;align-items:baseline;gap:10px"><span style="font-size:13px;font-weight:700">${t('int_friction_by_cp')}</span>
      <span style="font-size:10px;color:var(--color-accent-700);margin-left:auto;white-space:nowrap">${t('int_click_row_filter')}</span></div>
...
function intelFrictionCopilotHtml(st){
  ...
  const head=`<div style="display:flex;align-items:center;gap:8px">
    ...
    <span style="font-size:12.5px;font-weight:700">${t('int_copilots_read')}</span>
    <span style="font-size:9px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;padding:2px 7px;border-radius:999px;background:var(--color-accent-100);color:var(--color-accent-700)">${t('int_optional_ai')}</span>
  </div>`;
  let body;
  if(ai&&ai.busy&&ai.key===key){
    body=`<div style="display:flex;align-items:center;gap:9px;font-size:12px;color:var(--color-neutral-600);padding:2px 0">
      <span class="live-ping" style="width:7px;height:7px;border-radius:50%;background:var(--accent-solid,var(--color-accent));flex:none"></span>${t('int_reading_figures')}</div>`;
    ...
    body=`<div class="igf-ai-read" style="font-size:12.5px;line-height:1.7;color:var(--color-neutral-800)">${ai.html}</div>
      <div style="display:flex;align-items:center;gap:10px;margin-top:8px;padding-top:8px;border-top:1px dashed var(--color-divider);font-size:10px;color:var(--color-neutral-500)">
        <span style="flex:none;font-size:9px;font-weight:700;padding:1px 7px;border-radius:999px;background:var(--st-amber-bg);color:var(--st-amber-fg)">${t('int_ai_commentary')}</span>
    ...
    body=`<div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">
      ...
      <button id="igf-ai-ask" style="border:0;background:none;cursor:pointer;font:inherit;font-size:11.5px;font-weight:700;color:var(--color-accent-700)">${t('int_try_again')}</button>
    </div>`;
  }else{
...
function igExplainCard(id){
  ...
  <div class="rounded-xl border border-brand-100 bg-white p-3" data-ig-hoverid="${c.id}">
    ...
    <div class="mt-2 flex items-center gap-1.5">
      <button data-ig-ws="${c.id}" class="flex-1 rounded-lg bg-brand-900 text-white px-3 py-1.5 text-[11.5px] font-600 hover:bg-brand-800 transition">${t('int_open_workspace')}</button>
      <button data-ig-cmp="${c.id}" class="rounded-lg border ${intel.compareSel.includes(c.id)?'border-brand-500 bg-brand-50 text-brand-700':'border-brand-200 text-brand-700 hover:border-brand-400'} px-2.5 py-1.5 text-[11.5px] font-600 transition" title="${t('int_stage_for_compare')}">${intel.compareSel.includes(c.id)?'✓ Comparing':'+ Compare'}</button>
    </div>
  </div>`;
}
...
function renderIntelDock(){
  ...
  if(!intel.dockOpen){
    dock.innerHTML=`
      <button id="igd-expand" title="${t('int_open_panel')}" class="h-full w-full flex flex-col items-center pt-3 gap-2 text-gold-500 hover:bg-brand-50/60 transition">
        ${icon('sparkle','w-4 h-4')}<span class="text-[9px] font-mono text-ink/40 [writing-mode:vertical-rl]">${t('int_copilot_panel')}</span>
      </button>`;
  ...
  dock.innerHTML=`
    <div class="flex items-center gap-2 px-3.5 py-3 border-b border-hair shrink-0">
      ...
      <span class="font-display font-700 text-[13px] text-ink flex-1">${t('int_intelligence_panel')}</span>
      ${(()=>{ const b=(typeof copilotBrainInfo==='function')?copilotBrainInfo():{live:false,get label(){ return t('int_basic_mode'); },hint:''};
        return b.live
      ...
      ${intel.history.length?`<button id="igd-history-clear" title="${t('int_clear_conversation')}" class="h-6 w-6 grid place-items-center rounded-lg text-ink/40 hover:text-rose-600 hover:bg-brand-50 transition">${icon('trash','w-3.5 h-3.5')}</button>`:''}
      ...
      <button id="igd-collapse" title="${t('int_collapse_panel')}" class="h-6 w-6 grid place-items-center rounded-lg text-ink/40 hover:text-ink hover:bg-brand-50 transition text-[13px]">›</button>
    </div>
    ${intel.lenses.length?`
    <div class="px-3.5 py-2 border-b border-hair shrink-0 flex flex-wrap items-center gap-1.5">
      ${intel.lenses.map(l=>`
        <span data-lens-hover="${l.id}" class="ig-lens inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10.5px] font-mono cursor-pointer ${l.on?'border-brand-500 bg-brand-50 text-brand-700':'border-line bg-white text-ink/40'}">
          ...
          <button data-lens-x="${l.id}" title="${t('int_remove_lens')}" class="hover:text-rose-600">✕</button>
        </span>`).join('')}
      <button id="igd-clear" class="text-[10.5px] font-600 text-brand-600 hover:text-brand-800 ml-auto">${t('int_clear_all')}</button>
    </div>`:''}
    ...
    <div class="px-3.5 py-2 border-t border-hair shrink-0 flex items-center gap-2 bg-brand-50/40">
      <span class="text-[11px] text-brand-800/70 flex-1 min-w-0 truncate">${t('int_comparing')} <b class="text-brand-900">${intel.compareSel.length}</b>: ${intel.compareSel.map(id=>igEsc(getContract(id)?.name||id)).join(', ')}</span>
      <button id="igd-cmp-clear" class="text-[10.5px] font-600 text-ink/50 hover:text-ink">${t('int_clear')}</button>
    ...
    <div class="p-3 border-t border-hair shrink-0 relative">
      <input id="igd-input" placeholder="${t('int_ask_portfolio')}" class="w-full rounded-xl border border-inputln bg-white pl-3.5 pr-16 py-2.5 text-[13px] outline-none focus:border-brand-600 focus:ring-[3px] focus:ring-[rgba(11,122,95,.1)] transition"/>
      <button id="igd-go" class="absolute right-[18px] top-1/2 -translate-y-1/2 rounded-lg bg-brand-600 text-white px-3 py-1.5 text-[11px] font-600 hover:bg-brand-700 transition">${t('int_ask')}</button>
    </div>`;
  ...
  document.getElementById('igd-history-clear')?.addEventListener('click',()=>{
    ...
    if(typeof toast==='function') toast(t('int_conversation_deleted'));
  });
}
