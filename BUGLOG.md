...
The audit's step 1, built on the owner's word, with the one-door refusal LIFTED BY NAME: *"Implement this step 1 but do not delete the feature in image 2."* So the act keeps its More-menu row and gains a button where the work starts.
Defects found
- THE COLUMN'S BUTTON WAS DRAWN, LIVE-LOOKING AND DEAD on the first build, and only a real press found it: `[data-rl-prepare]` was bound in renderRedline's head block, and the cards column is painted into its mount FURTHER DOWN THAT SAME FUNCTION — so the query ran before the button existed. Not a timeout and not an error; the button was there, not disabled, and pressing it raised nothing. The handler moved to `rlWireClauseTools` (after the mount, holding the contract and the repaint) and is `querySelectorAll`, never `querySelector`: the singular form bound the menu row alone. One handler over both doors is what makes two doors one act.
- Both buttons carried the SAME PENCIL, photographed before it was fixed: the shared builder writes one and the pencil is also the product's own sign for "edit a clause", so the pair read as two of the same act. Dropped from the filled button in this home only; the menu row is untouched.
- Two nets that passed on nothing, caught and made measurements: `bodyOf()` counts braces from the first one it meets, which on a signature carrying `opts = {}` is the DEFAULT — it answered '{}' and four claims passed on an empty string (sliceFn now cuts to the next top-level declaration); and the counterparty check read a global the page does not have, so the evaluate threw, the guard swallowed it and both claims passed on null (the id is an argument now, and our own seat is the CONTROL beside it).
Noticed, not fixed
- redline-verify check 5 ("no clause carries a Copilot button — and no menu follows a highlight") is RED, and was red before this session: proved by running the file in a worktree at unmodified main, 217/218 both ways, the same items reported. It belongs to the comments rounds' three verbs, not to this.
- The empty column's heading still reads "No changes on the table." — the mock said "Nothing on the table yet". The existing key was kept; changing what the product SAYS was not part of the ask.
- The audit's other open asks are untouched and still the owner's to rule on: a one-line summary at the head of the column (a new strip), and the share dialog's retired one-press send.
- AND ONE MORE DEFECT, found by the full suite rather than by the feature's own files: "Edit a clause" called `rlOpenClauseEditor` itself, from `rlWireClauseTools`, where the contract was equally to hand — and f245 (1) and (19) both went red on one claim, that exactly ONE place in js/views/negotiation.js may open that page. `openEditor` in `wireNegotiationTab` is the named reading of what a press onto the clause editor MEANS, and three doors already shared it; this is the fourth caller, not a fifth answer. Moved there, f295's own claim re-pointed to it, the browser press re-driven, and the full suite finished 6740/6740 with lint at 0 errors.

### The negotiation is the contract's workspace — one card, two pages (Young ruled 12 Sep 2026)
Four answers settled before a line moved: the crumb says Contract Workspace / MK-390; the head's acts stay as they are; the counterparty comes off the title; the card takes the room's own height. Measured on a real page first and after every step.
Defects found
- None pre-existing. What the change itself broke and how it was caught, each by a net rather than by reading: (1) **the first painted glyph moved 4px up** — the workbench head's top padding was a calculation compensating for a ONE-LINE head whose first glyph rode a 28px acts row; with a crumb above, the compensation was wrong by exactly what it used to correct. pages-read-alike 8 reported "Negotiation page 10px against Home 14px". Plain `var(--page-pad-t)`, the room's own, and all fourteen pages line up again. (2) **the card was 14px short** — the head had `gap:0` for rows because it had no rows; it has three now, and the room's 6px is what it takes. (3) **the last 2px** was `.room-headsub`'s 1px top margin against `.room-sub`'s 3px. (4) **f236 caught a backtick pair inside a CSS comment** in a file that returns its CSS from a template literal — this file's own recorded trap, walked again by the note explaining the fix above. (5) The first draft of a TEST comment carried a comment terminator followed by a brace and broke the test file at parse — the same trap in the other language.
- THE CARD IS NOT `#ws-head`: the room spends 16px of its card inside `.room-band` and the workbench inside its own padding, so comparing those two boxes compared two different things and read "equal" at 129.8 while the visible cards differed by 14. The measure is the shell bar to the bottom of the facts row: 145.8 on both, crumbs at one top.
- Cost to the contract on the negotiate page, stated because refusal 3 is a number: first ink 286.2 → 305.8, **19.6px**, which is the owner's own ruling ("use the same height as contract workspace"). That page still gives the paper 40.6px more than the room does.
Noticed, not fixed
...
### The audit page changed from outside this session, and the PDF was reissued (12 Sep 2026, on a republish wake)
NOT A BUILD, and no product code was touched. The watch on the audit artifact woke this session at 15:11 UTC: the page had been republished eight hours after this session's own version 3. Re-read first, then diffed against the copy this session published.
Defects found
- None, in the product or in the page. The change is five replaced lines and one added line, all ONE THEME: recommendation 2, Copilot's rail, NARROWED because it duplicated two features that are already built — Prepare redlines files a redline for every playbook gap in one press, and the clause editor's rail already carries a Playbook scan tab listing that clause's standards with a verb on each. CHECKED AGAINST THE CODE BEFORE IT WAS BELIEVED (the scan tab exists by name, the per-clause deviations reading exists, the prepare runner walks the whole contract): the narrowed claim is correct, and better grounded than the one this session published. What survives is a default and two facts — open the rail on the scan tab where the clause has findings, and let the greeting carry only what neither built feature already says.
- NOTHING WAS MERGED BACK, because this session held no pending edits to that page. A republish wake is not automatically work owed; the question it asks is "is my copy behind", and the answer here was yes with nothing of mine to preserve. The local publishable source now matches the live version, with the service's own skeleton stripped back off so a future republish cannot nest a second one.
- THE DELIVERED PDF WAS SUPERSEDED BY THE CHANGE and has been reissued from the current page: 27 pages where the first was 26, the extra page being the added text. Rendered through the same refusal that fixed the first attempt — it will not print unless the count of decoded pictures equals the count on the page (14/14). The page count was MEASURED off the file, not assumed.
Noticed, not fixed
- The first PDF handed over at 08:59 carries the older text. Anyone holding that copy is one section out of date on Copilot's rail and nothing else.
- A republish arriving from outside a session is worth a habit: re-read, diff against what was published, and check any NEW claim against the code before treating the page as right. The narrowing here was correct, but the diff is the only reason that is known rather than assumed.
