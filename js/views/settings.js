        ...
        <section style="${cardStyle}">
          ...
          <div style="margin-top:12px;padding-top:12px;border-top:1px solid var(--color-divider)">
            ...
            <label style="display:flex;align-items:flex-start;gap:8px;margin-top:9px;font-size:11px;color:var(--color-neutral-700);line-height:1.45;cursor:pointer">
              ...
              <span><b>${i18t('set_thorough_extraction')}</b> — read the whole contract in overlapping chunks instead of one pass.
              <span style="color:var(--st-amber-fg)">This runs one deep-tier Copilot call per ~30,000 characters, so a long agreement can cost several times a normal extraction.</span> ${i18t('set_preflight')}</span></label>
          ...
          <div style="margin-top:12px;padding-top:12px;border-top:1px solid var(--color-divider)">
            ...
            <p style="font-size:10.5px;color:var(--color-neutral-600);margin:2px 0 8px;line-height:1.5">${i18t('set_usd_per')} <b>million tokens</b>. Spend is metered against these, so a stale table silently under-reports the bill — check them against Anthropic's published pricing when you upgrade a model.</p>
          ...
          <div style="font-size:11.5px;line-height:1.5;color:${r.northStar.withinSevenDays===true?'var(--st-green-fg)':r.northStar.withinSevenDays===false?'var(--st-amber-fg)':'var(--color-neutral-700)'}">
            ${r.northStar.firstSendDays==null
              ...
              : r.northStar.withinSevenDays
                ? `<b>${i18t('set_north_star')}</b> first send ${r.northStar.firstSendDays===0?'on day one':`after ${r.northStar.firstSendDays} day${r.northStar.firstSendDays===1?'':'s'}`}.`
