...
function mHomeHtml(){
  ...
  const needsHtml = needs.length ? `
      ...
      ${needs.map(n=>`
        <button class="m-row" data-m-open="${mEsc(n.c.id)}" style="align-items:flex-start">
          ...
          <span style="flex:1;min-width:0">
            ...
            <span style="display:block;font-size:14px;margin-top:5px;line-height:1.45;color:${n.tone}">${mEsc(n.reason)}</span>
          </span>
    ...
    <div class="m-card" style="margin:0 16px;padding:30px 20px;text-align:center">
      ...
      <div style="font-size:18px;font-weight:600">${t('m_nothing_needs_you')}</div>
      <div class="m-note" style="margin-top:4px">${t('m_nothing_needs_sub')}</div>
    </div>`;

  const expHtml = expiring.length ? `
    <div class="m-lbl" style="margin:14px 16px 6px">${t('m_expiring_next')}</div>
    <div class="m-card m-list" style="margin:0 16px">
  ...
  return `
    <div class="m-pagehead">
      ...
      <div class="m-title">${t('m_portfolio')}</div>
    ...
    <div class="m-scroll">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin:16px 16px 6px">
        <span class="m-lbl">${t('m_your_metrics')}</span>
        <button class="m-kpi-gear" data-m-act="kpis" aria-label="${t('m_choose_metrics')}">
      ...
      <div class="m-card m-list" style="margin:14px 16px 24px">
        <button class="m-row" data-m-act="portfolio">
          <span style="flex:1;font-size:16px;font-weight:500;color:var(--color-accent-700)">${t('m_portfolio')}</span>
          <span style="font-size:14px;color:var(--color-neutral-600)">${t('m_figures_streams')}</span>
          ${M_CHEV}
        </button>
        <button class="m-row" data-m-act="more">
          <span style="flex:1;font-size:16px;font-weight:500;color:var(--color-accent-700)">${t('m_more')}</span>
          <span style="font-size:14px;color:var(--color-neutral-600)">${t('m_more_sub')}</span>
          ${M_CHEV}
        </button>
      </div>
    </div>`;
...
function mKpiSheetHtml(){
  ...
  return `
    ...
    <div class="m-sheet-title">${t('m_show_metrics')}</div>
    <div class="m-sheet-note">${t('m_same_choice')}</div>
    <div class="m-card m-list">${rows}</div>
    <button class="m-btn m-btn-quiet" style="margin-top:12px" data-m-act="kpi-reset">${t('m_reset_four')}</button>
    <button class="m-btn m-btn-quiet" style="margin-top:8px" data-m-act="close-sheet">${t('m_done')}</button>`;
}
...
function mContractsHtml(){
  ...
  const STATUS_CHIPS = [
    { k:'all', get label(){ return t('m_all'); } },
    ...
    <div class="m-card m-list" style="margin:16px">
      ${rows.map(c=>`
        <button class="m-reg-row" data-m-open="${mEsc(c.id)}">
          ...
          <span style="display:flex;gap:10px;margin-top:5px;font-size:14px">
  ...
  : `
    <div class="m-card" style="margin:16px;padding:34px 20px;text-align:center">
      ...
      ${total?`<button class="m-btn m-btn-quiet" style="margin-top:14px" data-m-act="clear-filters">${t('m_clear_all_filters')}</button>`:''}
    </div>`;
    ...
    <div class="m-pagehead" style="padding-bottom:0">
      <div style="display:flex;align-items:flex-start;gap:8px;margin-bottom:10px">
        <div style="flex:1;min-width:0">
          <div class="m-title" style="margin-top:0">${t('m_contracts')}</div>
          <div class="m-sub">${t('m_filter_and_act')}</div>
        </div>
        <button class="m-new" data-m-act="new" aria-label="${t('m_new_contract')}">
      ...
      <div class="m-search">
        ...
        <input id="m-reg-q" value="${mEsc(R.query||'')}" placeholder="${t('m_search_contracts')}" autocomplete="off">
      </div>
...
function mNewSheetHtml(){
  ...
  const libRows = lib.map(t=>`
    <button class="m-row" data-m-newlib="${mEsc(t.id)}">
      <span style="flex:1;min-width:0">
        ...
        <span class="m-row-sub">${t('m_one_click')}</span>
      </span>
  ...
  const routes = `
    <button class="m-row" data-m-act="new-wizard">
      <span style="flex:1;min-width:0">
        <span class="m-row-name" style="font-weight:500">${t('m_draft_from_template')}</span>
        <span class="m-row-sub">${t('m_hati_standard')}</span>
      </span>${M_CHEV}
    </button>
    <button class="m-row" data-m-act="new-upload">
      <span style="flex:1;min-width:0">
        <span class="m-row-name" style="font-weight:500">${t('m_upload_received')}</span>
        <span class="m-row-sub">${t('m_their_word_pdf')}</span>
      </span>${M_CHEV}
    </button>
    <button class="m-row" data-m-act="new-import">
      <span style="flex:1;min-width:0">
        <span class="m-row-name" style="font-weight:500">${t('m_import_many')}</span>
        <span class="m-row-sub">${t('m_import_many_sub')}</span>
      </span>
      <span style="flex:none;font-size:14px;color:var(--color-neutral-600)">${t('m_computer')}</span>
    </button>`;
  return `
    <div class="m-grab"></div>
    <div class="m-sheet-title" style="margin-bottom:10px">${t('m_new_contract')}</div>
    ${lib.length?`<div class="m-capline" style="margin-bottom:6px">${t('m_company_standards')}</div>
      <div class="m-card m-list" style="background:var(--color-bg)">${libRows}</div>`:''}
    <div class="m-capline" style="margin:${lib.length?'14px':'0'} 0 6px">${t('m_other_ways_in')}</div>
...
function mApprovalsHtml(){
    ...
    <div class="m-card" style="margin-bottom:12px">
      ...
      <div style="border-top:1px solid var(--color-divider);padding:14px">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px 14px">
          <div><div class="m-note">${t('m_counterparty')}</div><div style="font-size:16px;font-weight:500;margin-top:1px">${mEsc((typeof cParty==='function'?cParty(c):c.counterparty)||'—')}</div></div>
          <div><div class="m-note">${t('m_value')}</div><div style="font-size:16px;font-weight:600;margin-top:1px">${mEsc(mMoney(c))}</div></div>
        ...
        <div style="display:flex;gap:10px;margin-top:14px">
          <button class="m-btn m-btn-primary" style="flex:1.4" data-m-approve="${mEsc(c.id)}">${t('m_approve')}</button>
          <button class="m-btn" style="flex:1;border-color:var(--st-ruby-line);color:var(--st-ruby-fg)" data-m-reject="${mEsc(c.id)}">${t('m_reject')}</button>
        </div>
        ${rejecting?`
        <div style="margin-top:12px">
          <div style="font-size:14px;font-weight:600;margin-bottom:5px">${t('m_why_rejecting')}</div>
          <textarea class="m-area" id="m-reject-why" rows="3" placeholder="${t('m_reason_goes_back')}"${s.apprErr?' style="border-color:var(--danger)"':''}>${mEsc(s.apprWhy||'')}</textarea>
          ${s.apprErr?`<div class="m-err">${t('m_give_reason')}</div>`:''}
          <button class="m-btn m-btn-danger" style="margin-top:10px" data-m-reject-send="${mEsc(c.id)}">${t('m_reject_send_back')}</button>
        </div>`:''}
      </div>`:''}
    </div>`;
  }).join('');
  ...
  return `
    <div class="m-pagehead">
      <div class="m-title">${t('m_approvals')}</div>
    ...
    <div class="m-scroll">
      <div style="margin:16px">
        ...
        <div class="m-card" style="padding:24px;text-align:center">
          <div style="font-size:16px;font-weight:600">${t('m_no_approvals')}</div>
          <div class="m-note" style="margin-top:3px">${t('m_new_requests_here')}</div>
        </div>`}
      </div>
    </div>`;
}
...
function mPortfolioHtml(){
  ...
  const rows = [];
  if(money && D) rows.push({ get label(){ return t('m_total_value'); }, value: fmtMoneyShort(D.m.totalValue) });
  if(D) rows.push({ get label(){ return t('m_agreements'); }, value: String((typeof agreementsIn==='function'?agreementsIn(D.cs):D.cs).length) });
  stages.forEach(s=>rows.push({ label:s.label, value:String(s.n) }));
  if(D) rows.push({ get label(){ return t('m_expiring_90'); }, value:String((D.exp90||[]).length) });
  if(D) rows.push({ get label(){ return t('m_high_risk'); }, value:String((D.highRisk||[]).length) });
  ...
  return `
    <div class="m-pagehead" style="display:flex;align-items:center;gap:4px;padding:8px 8px">
      ...
      <div style="font-size:19px;font-weight:600;font-family:var(--font-heading,inherit)">${t('m_portfolio')}</div>
    </div>
    <div class="m-scroll">
      <div class="m-card m-list" style="margin:16px">
        ${rows.map(r=>`<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:13px 14px">
          <span style="font-size:14px;color:var(--color-neutral-600)">${mEsc(r.label)}</span>
          <span style="font-size:16px;font-weight:600">${mEsc(r.value)}</span>
        </div>`).join('')}
      </div>
      <div class="m-note" style="margin:0 16px 24px">${t('m_reports_desk_work')}</div>
    </div>`;
}
...
function mWireScreen(root){
  ...
  root.querySelectorAll('[data-m-kpi-toggle]').forEach(b=>b.addEventListener('click',()=>{
    ...
    if(cur.includes(id)){
      if(cur.length<=1){ if(window.toast) toast(t('m_keep_one_metric'),'err'); return; }
