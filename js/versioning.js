...
function diffHtml(aStr, bStr){
  ...
  return wordDiff(aStr,bStr).map(p=>
    p.t==='eq' ? esc(p.text)
    : p.t==='add' ? `<ins style="background:var(--st-green-bg);color:var(--st-green-fg);text-decoration:none;border-radius:var(--radius);padding:0 1px">${esc(p.text)}</ins>`
    : `<del style="background:var(--st-ruby-bg);color:var(--st-ruby-fg);border-radius:var(--radius);padding:0 1px">${esc(p.text)}</del>`).join('');
}
...
const _diffLegend = `<span style="display:inline-flex;align-items:center;gap:8px"><ins style="background:var(--st-green-bg);color:var(--st-green-fg);text-decoration:none;border-radius:var(--radius);padding:0 4px">added</ins><del style="background:var(--st-ruby-bg);color:var(--st-ruby-fg);border-radius:var(--radius);padding:0 4px">removed</del></span>`;
const _diffBox = (a,b)=>`<div class="scroll-thin" style="border:1px solid var(--color-divider);border-radius:var(--radius);background:var(--color-surface);padding:14px 16px;font-size:14px;line-height:1.85;color:var(--color-doc-text);max-height:56vh;overflow-y:auto;white-space:pre-wrap;font-family:var(--font-body)">${diffHtml(a,b)}</div>`;
...
function openDiffModal(aText, bText, labelA, labelB){
  ...
  openModal(`
    <div style="padding:20px 22px">
      ...
      <p style="font-size:13px;color:var(--color-neutral-600);margin:0 0 12px;display:flex;flex-wrap:wrap;gap:10px;align-items:center">${_statLine(st)} · ${_diffLegend}</p>
...
function openCompareModal(c){
  ...
  const selStyle='font:inherit;font-size:14px;border:1px solid var(--color-divider);background:var(--color-surface);padding:6px 8px;border-radius:var(--radius);color:inherit;min-width:0;flex:1';
  ...
  const structuredBox=(aText,bText)=>{
    ...
    return `<div class="cmp-doc" style="font-size:14px;line-height:1.75;border:1px solid var(--color-divider);border-radius:var(--radius);background:var(--color-surface);color:var(--color-doc-text);padding:14px 18px;max-height:62vh;overflow:auto">${html}</div>`;
  };
  const foldUnchanged=()=>{
  ...
  const segBtn=(k,label,on)=>`<button data-cmp-mode="${k}" style="border:0;border-radius:var(--radius);padding:5px 12px;font:inherit;font-size:13px;font-weight:600;cursor:pointer;background:${on?'var(--accent-solid,var(--color-accent))':'none'};color:${on?'#fff':'var(--color-neutral-600)'}">${label}</button>`;
  openModal(`
    <div style="padding:20px 22px">
      ...
      <div id="cmp-legend" style="font-size:13px;color:var(--color-neutral-600);margin-bottom:10px"></div>
      <style>
        #cmp-out .cmp-line{margin:0 0 6px}
        #cmp-out h4.cmp-line{font-weight:700;font-size:14px;letter-spacing:.02em;margin:14px 0 7px}
        ...
        #cmp-out .cmp-line-ins{background:color-mix(in srgb,var(--st-green-bg) 45%,transparent);border-radius:var(--radius)}
        #cmp-out .cmp-line-del{background:color-mix(in srgb,var(--st-ruby-bg) 40%,transparent);border-radius:var(--radius)}
        #cmp-out .cmp-fold{display:block;width:100%;text-align:center;border:1px dashed var(--color-divider);background:var(--color-neutral-100);color:var(--color-neutral-600);border-radius:var(--radius);font:inherit;font-size:12px;font-weight:600;padding:4px;margin:8px 0;cursor:pointer}
      ...
      <div id="cmp-out" style="font-size:13px"><span style="color:var(--color-neutral-500)">${i18t('ve_pick_two')} <b>${i18t('ve_compare')}</b> ${i18t('ve_to_see_changes')}</span></div>
      ...
      <div style="display:flex;align-items:center;gap:8px;margin-top:14px;flex-wrap:wrap">
        ${canSnap?`<button id="cmp-snap-now" class="ui-btn" style="font-size:13px;padding:5px 11px" title="${i18t('ve_save_wording_now')}">${icon('plus','w-3 h-3')} Snapshot now</button>
        <button id="cmp-restore" class="ui-btn" style="font-size:13px;padding:5px 11px" hidden></button>`:''}
  ...
  const runCum=()=>{
    ...
    const trail=moved.length
      ? `<div style="font-size:13px;line-height:1.9;color:var(--color-neutral-700);border:1px solid var(--color-divider);border-radius:var(--radius);padding:9px 12px;margin-bottom:10px"><b>${i18t('ve_clauses_moved')}</b> ${moved.slice(0,8).map(m=>`${m.label} ×${m.n}`).join(' · ')}${moved.length>8?` · +${moved.length-8} more`:''}</div>`
      : '';
...
function reviewProposedRound(c, n){
  ...
  const blockRow=b=>{
    ...
    return `
    <div style="border:1px solid var(--color-divider);background:var(--color-surface);border-radius:var(--radius);padding:11px 13px">
      ${b.context?`<div style="font-size:12px;color:var(--color-neutral-500);margin-bottom:6px">…${e(b.context)}</div>`:''}
      <div style="font-size:14px;line-height:1.7">
        ${b.before.trim()?`<del style="background:var(--st-ruby-bg);color:var(--st-ruby-fg);border-radius:var(--radius);padding:0 2px">${e(b.before.trim())}</del> `:''}
      ...
      ${ask?`<div style="margin-top:8px;border-left:2px solid var(--color-accent-300);background:var(--color-accent-100);border-radius:var(--radius);padding:7px 10px">
      ...
      <div style="display:flex;gap:6px;margin-top:9px;align-items:center">
        ...
        <button data-dec="reject" data-for="${b.id}" class="ui-btn" style="font-size:13px;padding:5px 12px">${i18t('ve_reject')}</button>
        <span data-state="${b.id}" style="margin-left:auto;font-size:12px;font-weight:600"></span>
      </div>
      <input data-reply="${b.id}" type="text" placeholder="${i18t('ve_your_reply_optional')}" style="width:100%;margin-top:7px;border:1px solid var(--color-divider);border-radius:var(--radius);padding:6px 9px;font:inherit;font-size:13px;background:var(--color-bg);outline:none"/>
    </div>`;};
  openModal(`
    <div style="height:100%;display:flex;flex-direction:column;min-height:0">
      <div style="flex:none;padding:20px 26px 14px;border-bottom:1px solid var(--color-divider)">
        <div style="${COL}">
          <div style="display:flex;align-items:center;gap:9px;flex-wrap:wrap">
            ...
            <span style="font-size:11px;font-weight:700;letter-spacing:.09em;text-transform:uppercase;background:var(--st-amber-bg);color:var(--st-amber-fg);border-radius:var(--radius);padding:3px 9px">${i18t('ve_round_open',{n})}</span>
          </div>
        ...
        <div style="${COL}">
          ${blocks.length?`
            <div style="display:flex;align-items:center;gap:9px;flex-wrap:wrap;margin-bottom:11px">
              <span style="font-size:13px;font-weight:600">${i18tn('ve_decide_each',blocks.length,{n:blocks.length})}</span>
              <span style="flex:1"></span>
              <button id="pr-all-acc" class="ui-btn" style="font-size:13px;padding:5px 11px">${i18t('ve_accept_all')}</button>
          ...
          ${r.comment?`<div style="margin-top:14px;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:var(--radius);padding:12px 16px">
          ...
          <label style="display:block;margin-top:14px">
            ...
            <textarea id="pr-reply" rows="2" placeholder="${e(i18t('ve_ph_reply'))}" style="width:100%;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:var(--radius);padding:9px 11px;font:inherit;font-size:14px;outline:none"></textarea>
          </label>
        </div>
      </div>
      <div style="flex:none;padding:14px 26px;border-top:1px solid var(--color-divider)">
        <div style="${COL};display:flex;align-items:center;gap:8px;flex-wrap:wrap">
          <span id="pr-summary" style="font-size:13px;color:var(--color-neutral-600)"></span>
          <span style="flex:1"></span>
