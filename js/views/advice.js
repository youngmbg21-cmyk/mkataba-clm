    ...
    <div data-adv-card="${r.id}" ${drag?'draggable="true"':''} class="q-card" style="background:var(--color-surface);border:1px solid var(--color-divider);border-radius:5px;box-shadow:var(--shadow-sm);padding:11px 12px;cursor:${drag?'grab':'pointer'};display:flex;flex-direction:column;gap:5px">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:6px">
        ...
        <span style="display:flex;align-items:center;gap:4px;flex:none">
          ${r.urgency==='priority'?`<span style="background:var(--st-amber-bg);color:var(--st-amber-fg);font-size:9.5px;font-weight:600;letter-spacing:.03em;padding:2px 8px;border-radius:999px">${t('adv_priority')}</span>`:''}
  ...
  const columnsHtml=groups.map(g=>`
    <div style="min-width:0;display:flex;flex-direction:column;min-height:0">
      ...
      <div data-adv-drop="${g.col.k}" class="pipe-col scroll-thin" style="background:color-mix(in srgb,var(--color-accent) 6%,transparent);border:1px solid var(--color-divider);border-radius:8px;padding:8px;display:flex;flex-direction:column;gap:8px;flex:1;min-height:0;overflow-y:auto">
        ${g.list.map(adviceCard).join('')||`<div style="border:1px dashed var(--color-divider);border-radius:4px;padding:22px 10px;text-align:center;font-size:11px;color:var(--color-neutral-500)">${t('adv_nothing_here')}</div>`}
      </div>
    </div>`).join('');
...
async function adviceMove(id, target){
  ...
  if(!canEdit()){ toast(t('adv_viewers_no_move'),'err'); return; }
...
function wireAdviceBoard(){
  ...
  document.getElementById('adv-link')?.addEventListener('click',async()=>{
    ...
    try{ await navigator.clipboard.writeText(link); toast(t('adv_intake_copied')); }
    catch(e){ openModal(`<div style="padding:22px 24px"><h2 style="font-family:var(--font-heading);font-weight:600;font-size:17px;margin:0 0 8px">${t('adv_public_intake')}</h2><textarea readonly rows="2" style="width:100%;border:1px solid var(--color-divider);background:var(--color-bg);border-radius:4px;padding:10px;font-size:11px;font-family:var(--font-mono);word-break:break-all">${link}</textarea><div style="margin-top:12px;text-align:right"><button class="ui-btn" onclick="closeModal()">Close</button></div></div>`); }
  });
}
...
function openAdviceModal(id){
  ...
  const notes=(r.notes||[]).slice().reverse().map(n=>`
    ...
    </div>`).join('')||`<div style="font-size:11px;color:var(--color-neutral-500)">${t('adv_no_notes')}</div>`;
  ...
  openModal(`
    <div style="padding:22px 24px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:14px">
        <div>
          <h6 style="margin:0 0 6px;font-size:10px;color:var(--color-neutral-600);letter-spacing:.08em;text-transform:uppercase">${t('adv_pipeline_history')}</h6>
          <div class="scroll-thin" style="max-height:150px;overflow-y:auto">${hist}</div>
        </div>
        <div>
          <h6 style="margin:0 0 6px;font-size:10px;color:var(--color-neutral-600);letter-spacing:.08em;text-transform:uppercase">${t('adv_internal_notes')}</h6>
          <div class="scroll-thin" style="max-height:150px;overflow-y:auto">${notes}</div>
        </div>
      </div>
      ${canEdit()?`
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:14px">
        <label style="display:block"><span style="display:block;font-size:10.5px;font-weight:600;color:var(--color-neutral-700);margin-bottom:4px;font-family:var(--font-mono)">${t('adv_assigned_counsel')}</span>
          <select id="adv-assignee" style="${selStyle}"><option value="">${t('adv_unassigned')}</option>
            ${members.map(u=>`<option value="${esc(u.name)}" ${r.assignee===u.name?'selected':''}>${esc(u.name)} (${ROLE_LABEL[u.role]})</option>`).join('')}</select></label>
        <label style="display:block"><span style="display:block;font-size:10.5px;font-weight:600;color:var(--color-neutral-700);margin-bottom:4px;font-family:var(--font-mono)">${t('adv_stage')}</span>
      ...
      <label style="display:block;margin-top:10px"><span style="display:block;font-size:10.5px;font-weight:600;color:var(--color-neutral-700);margin-bottom:4px;font-family:var(--font-mono)">${t('adv_add_note')}</span>
  ...
  document.getElementById('adv-copy-track').addEventListener('click',async()=>{
    try{ await navigator.clipboard.writeText(adviceTrackLink(r)); toast('Tracking link copied — send it to '+r.name); }catch(e){ toast(t('adv_could_not_copy'),'err'); }
  });
...
function openRateCardModal(){
  ...
  openModal(`
    <div style="padding:22px 24px">
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px"><span style="display:inline-flex;color:var(--color-accent)">${icon('coins')}</span>
        <h2 style="font-family:var(--font-heading);font-weight:600;font-size:18px;margin:0">${t('adv_published_rates')}</h2></div>
      ...
      <div class="table-scroll"><table style="width:100%;border-collapse:collapse">
        <thead><tr style="text-align:left;border-bottom:1px solid var(--color-divider);color:var(--color-neutral-600);font-size:9.5px;letter-spacing:.08em;text-transform:uppercase">
          <th style="padding:6px 10px 6px 0;font-weight:600">${t('adv_service')}</th><th style="padding:6px;font-weight:600">${jxCurrency()} / hr</th><th style="padding:6px;font-weight:600">${t('adv_hrs_min')}</th><th style="padding:6px;font-weight:600">${t('adv_hrs_max')}</th><th style="padding:6px 0 6px 6px;font-weight:600">${t('adv_days')}</th>
        </tr></thead>
        <tbody>${rows}</tbody>
      </table></div>
      <p style="font-size:10.5px;color:var(--color-neutral-500);margin:10px 0 0;line-height:1.5">${t('adv_rate_note')}</p>
      <div style="margin-top:14px;display:flex;justify-content:flex-end;gap:8px">
        <button class="ui-btn" onclick="closeModal()">${editable?'Cancel':'Close'}</button>
        ${editable?`<button id="rt-save" class="ui-btn ui-btn-primary">${t('adv_publish_rates')}</button>`:''}
      </div>
    </div>`,{maxWidth:'36rem'});
  document.getElementById('rt-save')?.addEventListener('click',async()=>{
    ...
    for(const s of Object.values(ADVICE_SERVICES)){
      ...
      if(Object.values(o).some(v=>!Number.isFinite(v)||v<1)){ toast(t('adv_positive_number'),'err'); return; }
      if(o.hoursMax<o.hoursMin){ toast(`${s.name}: max hours must be ≥ min hours`,'err'); return; }
      out[s.id]=o;
    }
    state.settings.adviceRates=out;
    await saveSettings();
    closeModal(); toast(t('adv_rates_published'));
...
function openAdviceIntakeModal(){
  ...
  openModal(`
    <div style="padding:22px 24px">
      <h2 style="font-family:var(--font-heading);font-weight:600;font-size:18px;margin:0 0 4px">${t('adv_log_request')}</h2>
      <p style="font-size:12px;color:var(--color-neutral-700);margin:0 0 14px;line-height:1.5">${t('adv_log_sub')}</p>
      <label style="display:block;margin-bottom:10px"><span style="display:block;font-size:10.5px;font-weight:600;color:var(--color-neutral-700);margin-bottom:4px;font-family:var(--font-mono)">${t('adv_service')}</span>
      ...
      <label style="display:block;margin-bottom:10px"><span style="display:block;font-size:10.5px;font-weight:600;color:var(--color-neutral-700);margin-bottom:4px;font-family:var(--font-mono)">${t('adv_what_need')}</span>
        <textarea id="ai-desc" rows="3" style="${inputStyle}"></textarea></label>
      <label style="display:flex;align-items:center;gap:8px;font-size:12px;color:var(--color-neutral-700);margin-bottom:4px"><input id="ai-priority" type="checkbox" style="width:15px;height:15px;accent-color:var(--color-accent)"/> ${t('adv_priority_option')}</label>
      <div style="margin-top:12px;display:flex;justify-content:flex-end;gap:8px">
        <button class="ui-btn" onclick="closeModal()">Cancel</button>
        <button id="ai-go" class="ui-btn ui-btn-primary">${t('adv_create_request')}</button>
      </div>
    </div>`,{maxWidth:'34rem'});
  document.getElementById('ai-go').addEventListener('click',async()=>{
    ...
    if(!p.name||!p.email||!p.description){ toast(t('adv_name_email_required'),'err'); return; }
    try{
