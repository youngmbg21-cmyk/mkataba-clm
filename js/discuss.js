...
function discussBubbleHtml(m, mine){
  ...
  return `
    <div style="display:flex;flex-direction:column;gap:2px;align-items:${isMine ? 'flex-end' : 'flex-start'}">
      ...
      <div style="max-width:92%;border:1px solid ${internal ? 'rgba(138,106,42,.35)' : (isMine ? 'var(--color-divider)' : 'var(--color-accent-300)')};background:${internal ? 'rgba(245,158,11,.08)' : (isMine ? 'var(--color-bg)' : 'var(--color-accent-100)')};border-radius:var(--radius);padding:8px 11px;font-size:13px;line-height:1.55;color:var(--color-neutral-800);white-space:pre-wrap">${e(m.body)}</div>
    </div>`;
}
...
function discussPanelHtml(opts){
  ...
  const body = groups.length ? `
    ...
    <p style="margin:0 0 13px;font-size:13px;line-height:1.55;color:var(--color-neutral-600)">Nothing here yet. Use this to ask a question or answer one — it does not change the contract and it does not open a round.</p>`;
  const options = (topics || []).map(t =>
    `<option value="${e(t.value)}">${e(t.label)}</option>`).join('');
  const composer = disabled ? `
    <div style="border:1px solid var(--color-divider);background:var(--color-bg);border-radius:var(--radius);padding:9px 12px;font-size:13px;line-height:1.55;color:var(--color-neutral-600)">${e(disabledNote || 'This conversation is closed.')}</div>` : `
    <div style="border-top:1px solid var(--color-divider);padding-top:12px">
      <label style="display:block;margin-bottom:7px">
        ...
        <select id="${idp}-topic" style="width:100%;font:inherit;font-size:13px;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:var(--radius);padding:7px 9px;color:inherit">${options}</select>
      </label>
      <textarea id="${idp}-body" rows="2" placeholder="${e(i18t('di_ph_would_you'))}" style="width:100%;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:var(--radius);padding:8px 11px;font:inherit;font-size:14px;outline:none;resize:vertical"></textarea>
      <div style="display:flex;align-items:center;gap:9px;margin-top:8px;flex-wrap:wrap">
        <span style="flex:1;min-width:140px;font-size:12px;color:var(--color-neutral-600);line-height:1.45">${i18t('di_sends_message')}</span>
        <button id="${idp}-send" class="ui-btn ui-btn-primary" style="flex:none;font-size:13px;padding:7px 14px">${i18t('di_send')}</button>
      </div>
  ...
  return `
    <div id="${idp}-panel" style="border:1px solid var(--color-divider);background:var(--color-surface);border-radius:var(--radius);padding:14px 18px;margin:0 0 18px;box-shadow:var(--shadow-sm)">
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:11px;flex-wrap:wrap">
        ...
        ${waiting ? `<span style="font-size:11px;font-weight:700;letter-spacing:.09em;text-transform:uppercase;background:var(--st-amber-bg);color:var(--st-amber-fg);border-radius:var(--radius);padding:2px 9px">${waiting} awaiting your reply</span>` : ''}
...
function discussPointReplyHtml(topic, messages, opts){
  ...
  if (disabled) return thread + (disabledNote ? `
    <div style="margin-top:7px;font-size:12px;color:var(--color-neutral-600)">${e(disabledNote)}</div>` : '');
  return `${thread}
    <div style="display:flex;gap:6px;align-items:center;margin-top:8px;flex-wrap:wrap">
      <textarea data-point-body="${e(idp)}" class="chat-field" rows="1" placeholder="${e(placeholder || 'Reply on this point — a sentence, not a redraft')}"
        style="flex:1;min-width:150px;border:1px solid var(--color-divider);border-radius:var(--radius);padding:7px 10px;font:inherit;font-size:13px;background:var(--color-surface);outline:none"></textarea>
      <button data-point-send="${e(idp)}" data-point-topic="${e(topic)}" data-point-label="${e((opts&&opts.label)||'')}" class="ui-btn" style="flex:none;font-size:12px;padding:6px 12px">${i18t('di_send')}</button>
    </div>
    <div data-point-out="${e(idp)}" style="margin-top:6px"></div>
    ${on.length && !theirTurn ? `<div style="margin-top:5px;font-size:12px;color:var(--color-neutral-500)">${i18t('di_sent_waiting')}</div>` : ''}`;
}
  ...
  document.querySelectorAll('[data-point-send]').forEach(btn => {
    ...
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (out) out.innerHTML = `<div style="border:1px solid var(--st-ruby-line);background:var(--st-ruby-bg);border-radius:var(--radius);padding:7px 10px;font-size:12px;line-height:1.5;color:var(--st-ruby-fg)"><b>${i18t('di_not_sent')}</b> ${(window.esc ? esc(e.message) : e.message) || 'Something went wrong.'} Your reply is still in the box.</div>`;
...
function wireDiscussPanel(opts){
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (out) out.innerHTML = `<div style="border:1px solid var(--st-ruby-line);background:var(--st-ruby-bg);border-radius:var(--radius);padding:9px 11px;font-size:13px;line-height:1.5;color:var(--st-ruby-fg)"><b>${i18t('di_not_sent')}</b> ${(window.esc ? esc(e.message) : e.message) || 'Something went wrong.'} Your message is still in the box.</div>`;
...
function discussDiscardBtnHtml(change, opts){
  ...
  return `<button type="button" class="discuss-discard ui-btn" data-discuss-discard="${id}"
    ...
    style="margin-left:auto;flex:none;font-size:12px;padding:3px 8px;line-height:1.4;
