  ...
  return `${thread}
    <div style="display:flex;gap:6px;align-items:center;margin-top:8px;flex-wrap:wrap">
      <textarea data-point-body="${e(idp)}" class="chat-field" rows="1" placeholder="${e(placeholder || 'Reply on this point — a sentence, not a redraft')}"
        style="flex:1;min-width:150px;border:1px solid var(--color-divider);border-radius:0;padding:7px 10px;font:inherit;font-size:13px;background:var(--color-surface);outline:none"></textarea>
  ...
  document.querySelectorAll('[data-point-send]').forEach(btn => {
    ...
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (out) out.innerHTML = `<div style="border:1px solid var(--st-ruby-line);background:var(--st-ruby-bg);border-radius:0;padding:7px 10px;font-size:12px;line-height:1.5;color:var(--st-ruby-fg)"><b>${i18t('di_not_sent')}</b> ${(window.esc ? esc(e.message) : e.message) || 'Something went wrong.'} Your reply is still in the box.</div>`;
...
function wireDiscussPanel(opts){
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (out) out.innerHTML = `<div style="border:1px solid var(--st-ruby-line);background:var(--st-ruby-bg);border-radius:0;padding:9px 11px;font-size:13px;line-height:1.5;color:var(--st-ruby-fg)"><b>${i18t('di_not_sent')}</b> ${(window.esc ? esc(e.message) : e.message) || 'Something went wrong.'} Your message is still in the box.</div>`;
