...
CARDS ARE SHUT UNTIL SOMEBODY OPENS THEM. Only the head (.rl-card-head — id, origin, status, two-line delta) toggles; everything below is a control (so a verb can never fold the card — the one property worth guarding). rlCardIsOpen answers from _rlCardChoice alone; nothing outside the card changes it. TESTS pressing a card must press its HEAD and RE-QUERY after (the toggle repaints; a held node is detached, class frozen, rect zero).

A SHUT CARD IS TWO AREAS, NOT ONE (owner-asked, 2026-08-11): the header block AND the action bar. The verbs moved OUT of .rl-card-body — the element display:none is applied to — into a sibling .rl-card-actions that NO rule folds, carrying with them the review verdict buttons and the two sentences that explain a MISSING verb (deskCardInsteadHtml, the review hold's "what now"): an action bar with a hole in it and the explanation folded away is worse than either. The fold now hides reading matter only — who filed it, who rewrote it, Why they asked, the reviewer's note, and the notes/reply composer. Both rules survive the move and outrank the layout: the action bar is a SIBLING of the head (a verb can still never fold the card), and a verb is VISIBLE PIXELS (f180). Applies to redlineChangeCardsHtml, which is also what the PHONE and the counterparty embed render; negoLiveCardsHtml (contract tab) is a different card and already keeps its acts visible and folds only its Discuss thread — deliberately untouched. Tests: f84/f89 (the design contract), f95 (one shared radius), f100b/e/f, redline-verify 14b/14c, card-collapse-verify (21, browser — the two states, and every control below the head proved inert).
