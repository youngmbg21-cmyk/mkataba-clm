  ...
  return `<div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
    ...
    ${AI_PLACEMENTS.map(x => {
      ...
      return `<button type="button" data-ai-prop-place="${e(p.id)}" data-place="${e(x)}"
        ...
        style="font:inherit;font-size:12px;line-height:1;cursor:pointer;border-radius:var(--radius);padding:4px 9px;
...
function aiProposalCardHtml(p){
  ...
  <div class="ai-proposal" data-ai-proposal="${e(p.id)}"
    style="border:1px solid ${done ? 'var(--color-divider)' : 'rgba(99,102,241,.35)'};background:${done ? 'var(--color-bg)' : 'var(--color-surface)'};
      border-radius:var(--radius);padding:12px 14px;display:flex;flex-direction:column;gap:9px;${done ? 'opacity:.72' : ''}">
    <div style="display:flex;align-items:center;gap:7px;flex-wrap:wrap">
      <span style="font-size:11px;font-weight:700;letter-spacing:.09em;text-transform:uppercase;
        background:rgba(99,102,241,.18);color:color-mix(in srgb,#6366f1 55%,var(--color-text));border-radius:var(--radius);padding:2px 8px">${i18t('ai_proposed_wording')}</span>
    ...
    ${p.editing
      ? `<textarea data-ai-prop-edit="${e(p.id)}" class="ai-suggestion-editor" spellcheck="true" rows="6"
          style="width:100%;font:inherit;font-size:14px;line-height:1.6;border:1px solid rgba(99,102,241,.35);border-radius:var(--radius);
    ...
    ${done ? '' : `<label style="display:block;border:1px solid var(--st-ruby-line);border-radius:var(--radius);padding:8px 10px;
        ...
        <textarea data-ai-prop-why="${e(p.id)}" rows="2" wrap="soft" spellcheck="true"
          ...
          style="box-sizing:border-box;width:100%;max-width:100%;min-height:44px;resize:vertical;border:1px solid var(--color-divider);border-radius:var(--radius);padding:7px 9px;font:inherit;font-size:13px;line-height:1.6;background:var(--color-surface);color:var(--color-text);outline:none;white-space:pre-wrap;overflow-wrap:anywhere">${e(p.why || '')}</textarea>
      </label>`}
    ${done
      ? `<div style="font-size:12px;font-weight:600;border-radius:var(--radius);padding:5px 9px;background:${tone[0]};color:${tone[1]}">${tone[2]}</div>`
...
function renderBriefSection(c){
  ...
  host.innerHTML=`
    <style>
      ...
      #brief-section .br-item{border-radius:var(--radius);padding:10px 12px;border:1px solid transparent}
      ...
      #brief-section .br-facts{display:grid;grid-template-columns:auto 1fr;gap:6px 14px;
        margin:12px 0;padding:11px 13px;background:var(--color-bg);border-radius:var(--radius);
