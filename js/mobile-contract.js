...
function mTermsHtml(c){
  ...
  /* ONE MARKER, BOTH SHELLS: the figures a reader has to check are bolded by
     the desktop's own pass (briefMark, which escapes before it marks), so the
  ...
  const briefBlock = b ? `${briefFig}
      <div class="m-card m-brief" style="margin:16px 16px 0;padding:14px">
        ...
        ${(b.watchouts||[]).length?`<ul style="margin:8px 0 0;padding-left:18px;list-style:none">${(b.watchouts||[]).map(w=>`<li style="font-size:14px;line-height:1.55;margin:6px 0;padding:8px 10px;border-radius:var(--radius);background:var(--st-amber-bg);border-left:3px solid var(--st-amber-dot)">${mk(w.point||'')}</li>`).join('')}</ul>`:''}
      </div>` : '';
  return `
    <div class="m-scroll">
      ${briefBlock}
...
function mHistHtml(c){
  ...
  return `
    <div class="m-scroll">
      ...
      ${v?`
      <div style="margin:12px 16px 0;display:flex;gap:10px;border-radius:var(--radius);padding:13px;animation:mFadeIn .3s ease;
...
function mOverflowSheetHtml(){
  ...
  return `
    ...
    ${items.map(i=>`
      <button class="m-row" data-m-act="${i.k}" style="padding:0 12px;border-radius:var(--radius)">
