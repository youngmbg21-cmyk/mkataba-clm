...
function pfChipsHtml(){
  ...
  return `<div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;margin-bottom:10px">
    ...
    ${chips.map(c=>`<button data-pf-unfilter="${c.k}" style="display:inline-flex;align-items:center;gap:6px;border:0;border-radius:var(--radius);padding:3px 10px;font:inherit;font-size:12px;font-weight:700;cursor:pointer;background:var(--color-accent);color:#fff">${pfEsc(c.l)} ✕</button>`).join('')}
...
function pfFigures(){
  ...
  const tile=(k,v,d,hero)=>`<div style="${PF_CARD};padding:12px 14px;gap:1px${hero?';background:var(--brand-hero,var(--color-accent-700));border-color:transparent;color:#fff':''}">
    <div style="font-size:11px;font-weight:700;letter-spacing:.09em;text-transform:uppercase;color:${hero?'var(--brand-hero-sub,#bde7e1)':'var(--color-neutral-600)'}">${k}</div>
  ...
  const rows=keys.map(k=>{
    ...
    return `<button data-pf-cat="${pfEsc(k)}" style="display:grid;grid-template-columns:1fr 92px;gap:9px;align-items:center;width:100%;text-align:left;border:0;background:${sel?'color-mix(in srgb,var(--color-accent) 10%,transparent)':'none'};border-radius:var(--radius);cursor:pointer;font:inherit;padding:${dense?'6px':'8px 6px'};border-bottom:1px solid var(--color-divider);opacity:${dim?'.42':'1'}">
      <span style="min-width:0">
        ...
        <span style="font-size:12px;color:var(--color-neutral-600)">${pfN(e.n,'contracts')} · ${pfShare(e.v,total)}</span>
...
function pfBiggest(){
  ...
  const th='text-align:left;font-size:11px;font-weight:700;letter-spacing:.09em;text-transform:uppercase;color:var(--color-neutral-600);padding:5px 8px;border-bottom:1px solid var(--color-divider);white-space:nowrap';
  ...
  const rows=keys.map(k=>{
    ...
    return `<tr data-pf-cp="${pfEsc(k)}" style="cursor:pointer;${sel?'background:color-mix(in srgb,var(--color-accent) 10%,transparent)':''}">
      <td style="${td};font-weight:600">${pfEsc(k)}
        <div style="font-size:12px;color:var(--color-neutral-600);font-weight:400">${pfEsc(pfCatLabel(e.cat))}${e.n>1?' · '+pfN(e.n,'contracts'):''}</div></td>
      ...
      <td style="${td};text-align:right;white-space:nowrap">${e.find
        ? `<span style="font-size:12px;font-weight:700;padding:1px 7px;border-radius:var(--radius);background:var(--st-amber-bg);color:var(--st-amber-fg)">${e.find}</span>`
...
function pfFindings(){
  ...
  const body=`<div style="display:flex;flex-direction:column">${shown.map(({c,f})=>{
    ...
    return `<button data-pf-open="${pfEsc(c.id)}" style="display:flex;gap:9px;align-items:flex-start;width:100%;text-align:left;border:0;background:none;cursor:pointer;font:inherit;padding:8px 2px;border-bottom:1px dashed var(--color-divider)">
      <span style="flex:none;margin-top:2px;font-size:12px;font-weight:700;padding:1px 7px;border-radius:var(--radius);background:${t.bg};color:${t.fg};text-transform:uppercase;letter-spacing:.04em">${pfEsc(f.sev||'low')}</span>
...
function portfolioFrameHtml(){
  ...
  const nudge = uncounted.length ? `
    <div style="display:flex;gap:11px;align-items:flex-start;flex-wrap:wrap;margin-bottom:10px;padding:10px 13px;border-radius:var(--radius);background:var(--st-amber-bg);border:1px solid var(--st-amber-line);color:var(--st-amber-fg);font-size:13px;line-height:1.55">
      <span style="flex:1;min-width:220px"><b>${i18t('pf_uncounted_head',{n:uncounted.length})}</b> ${i18t('pf_uncounted_body')}</span>
      ${(typeof canEdit!=='function'||canEdit())?`<button data-pf-fixcats style="flex:none;border:1px solid currentColor;background:none;color:inherit;border-radius:var(--radius);padding:5px 11px;font:inherit;font-size:13px;font-weight:700;cursor:pointer">${i18t('pf_uncounted_fix')}</button>`:''}
    </div>` : '';
  ...
  <div>
    ...
    <div style="margin-top:2px;padding:9px 13px;border-radius:var(--radius);background:var(--color-neutral-100);font-size:12px;line-height:1.55;color:var(--color-neutral-600)">
    ...
    rows.slice(0,PF_HELD_ROWS).map(r=>{
      ...
      return `<tr data-pf-open="${pfEsc(r.id)}" style="cursor:pointer">
        <td style="${td};font-weight:600">${pfEsc(r.name)}
          <div style="font-size:12px;color:var(--color-neutral-600);font-weight:400">${r.retentionPct}% ${i18t('pf_ret_held_lc')}${due?' · '+i18t('pf_ret_due_back')+' '+pfEsc(pfDate(due)):''}</div></td>
        ...
        <td style="${td};width:1%"><span style="font-size:12px;font-weight:700;padding:1px 7px;border-radius:var(--radius);background:${pal.bg};color:${pal.fg};white-space:nowrap">${st.t}</span></td></tr>`;
    }).join('')}</tbody></table></div>`;
  const head=`<div style="display:flex;align-items:baseline;gap:8px;font-size:11px;font-weight:700;letter-spacing:.09em;text-transform:uppercase;color:var(--color-neutral-600);margin-bottom:6px">
...
function pfPromisesLive(){
  ...
  const body=`<div>${rows.slice(0,PF_PROMISE_ROWS).map(r=>{
    ...
    return `<button data-pf-open="${pfEsc(r.id)}" style="display:grid;grid-template-columns:1fr 104px;gap:9px;align-items:center;width:100%;text-align:left;border:0;background:none;cursor:pointer;font:inherit;padding:5px 3px;border-radius:var(--radius)">
      <span style="min-width:0"><span style="display:block;font-size:13px;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${pfEsc(r.name)}</span>
        <span style="display:block;position:relative;height:10px;margin-top:4px">
          <span style="position:absolute;left:${at(a)}%;width:${Math.max(at(b)-at(a),2)}%;top:2px;height:6px;border-radius:var(--radius);background:var(--color-accent-500,var(--color-accent))"></span>
...
function pfWonLost(){
  ...
  const stat=(k,v,d,col)=>`<div style="background:var(--color-neutral-100);border-left:3px solid ${col};border-radius:var(--radius);padding:7px 10px">
    <div style="font-size:11px;font-weight:700;letter-spacing:.09em;text-transform:uppercase;color:var(--color-neutral-600)">${k}</div>
    ...
    <div style="font-size:12px;color:var(--color-neutral-600)">${d}</div></div>`;
