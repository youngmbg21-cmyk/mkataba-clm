...
function aiProposalAnchorHtml(p){
  ...
  const line = s => `<div style="font-size:11px;color:var(--color-neutral-500);line-height:1.5">${s}</div>`;
  ...
  return `<div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
    ...
    ${AI_PLACEMENTS.map(x => {
      ...
      return `<button type="button" data-ai-prop-place="${e(p.id)}" data-place="${e(x)}"
        ...
        style="font:inherit;font-size:11px;line-height:1;cursor:pointer;border-radius:0;padding:4px 9px;
...
function aiProposalCardHtml(p){
  ...
  <div class="ai-proposal" data-ai-proposal="${e(p.id)}"
    ...
    <div style="display:flex;align-items:center;gap:7px;flex-wrap:wrap">
      ...
      ${p.clauseLabel ? `<span style="font-size:11px;color:var(--color-neutral-600);font-family:var(--font-mono)">${e(p.clauseLabel)}</span>` : ''}
      ${p.strict === false ? `<span title="The Copilot did not return the structured shape, so this is its whole reply treated as wording."
        style="font-size:10px;color:var(--st-amber-fg)">${i18t('ai_unstructured_reply')}</span>` : ''}
    </div>
    ${p.editing
      ? `<textarea data-ai-prop-edit="${e(p.id)}" class="ai-suggestion-editor" spellcheck="true" rows="6"
          style="width:100%;font:inherit;font-size:13px;line-height:1.6;border:1px solid rgba(99,102,241,.35);border-radius:0;
            padding:9px 11px;background:var(--color-surface);color:inherit;resize:vertical">${e(p.text)}</textarea>`
      : `<div class="ai-proposal-text" style="font-size:13px;line-height:1.65;white-space:pre-wrap;
    ...
    ${done ? '' : `<label style="display:block;border:1px solid var(--st-ruby-line);border-radius:0;padding:8px 10px;
        ...
        ${''/* A VERY LIGHT RED, ASKED FOR BY NAME (Young, 03 Aug 2026). On the
              ...
              the box invites. */}
        <span style="display:block;font-size:10px;font-weight:700;letter-spacing:.05em;text-transform:uppercase;color:var(--color-neutral-600);margin-bottom:3px">${i18t('ai_why_change_optional')}</span>
        <textarea data-ai-prop-why="${e(p.id)}" rows="2" wrap="soft" spellcheck="true"
          placeholder="${_aiEsc(i18t('ng_ph_reason_example'))}"
          style="box-sizing:border-box;width:100%;max-width:100%;min-height:44px;resize:vertical;border:1px solid var(--color-divider);border-radius:0;padding:7px 9px;font:inherit;font-size:12px;line-height:1.6;background:var(--color-surface);color:var(--color-text);outline:none;white-space:pre-wrap;overflow-wrap:anywhere">${e(p.why || '')}</textarea>
      </label>`}
    ${done
      ? `<div style="font-size:11px;font-weight:600;border-radius:0;padding:5px 9px;background:${tone[0]};color:${tone[1]}">${tone[2]}</div>`
      : `<div style="display:flex;gap:7px;flex-wrap:wrap">
          <button class="ui-btn ui-btn-primary" data-ai-prop-apply="${e(p.id)}" style="font-size:12px;padding:5px 11px">${i18t('ai_apply_redline')}</button>
          <button class="ui-btn" data-ai-prop-decline="${e(p.id)}" style="font-size:12px;padding:5px 11px">${i18t('ai_decline')}</button>
          <button class="ui-btn" data-ai-prop-edit-btn="${e(p.id)}" style="font-size:12px;padding:5px 11px">${p.editing ? 'Done editing' : 'Edit'}</button>
        </div>`}
  </div>`;
}
...
function renderBriefSection(c){
  ...
  host.innerHTML=`
    <style>
      ...
      #brief-section .br-lead{font-size:14px;line-height:1.7;color:var(--color-text)}
      ...
      #brief-section .br-quote{font-size:12px;line-height:1.5;color:var(--color-neutral-600);
      ...
      #brief-section .br-facts dd{margin:0;font-size:13px;line-height:1.6;color:var(--color-text)}
    </style>
...
function renewalCardHtml(c){
  ...
  return `<section id="renewal-section" class="kt-side-card" style="background:var(--color-surface);border:1px solid var(--color-divider);box-shadow:var(--shadow-sm);border-radius:0;padding:13px 15px">
    ...
    ${err?`<p style="margin:0 0 9px;padding:6px 8px;font-size:12px;line-height:1.5;background:var(--st-amber-bg);color:var(--st-amber-fg);border:1px solid var(--st-amber-line)">${_aiEsc(err)}</p>`:''}
    ${a?`
      ...
      <p style="margin:0 0 7px;font-size:13px;line-height:1.6">${_aiEsc(a.headline||'')}</p>
      ${(a.because||[]).length?`<ul style="margin:0 0 7px;padding-left:17px">${(a.because||[]).map(b=>`<li style="font-size:12px;line-height:1.5;margin:3px 0;color:var(--color-neutral-700)">${_aiEsc(b)}</li>`).join('')}</ul>`:''}
      ${(a.pushOn||[]).length?`<div style="font-size:12px;line-height:1.5;margin-bottom:7px"><b>${i18t('rn_push_on')}</b> ${_aiEsc((a.pushOn||[]).join(' · '))}</div>`:''}
    ...
    `:`<p style="margin:0 0 9px;font-size:12px;color:var(--color-neutral-600);line-height:1.55">${_aiEsc(i18t('rn_not_asked'))}</p>`}
