...
function openMetaReview(meta, onConfirm, opts={}){
  ...
  const checkLine = k => { const ck=checks[k]; if(!ck) return '';
    ...
    return `<span style="display:block;margin-top:4px;padding:5px 7px;border-radius:0;border:1px solid var(--st-amber-line);background:var(--st-amber-bg);color:var(--st-amber-fg);font-size:11px;line-height:1.5">
      ...
      <button type="button" data-mf-use="${k}" data-mf-val="${esc(ck.expected)}" style="border:0;background:none;padding:0;margin-left:4px;font:inherit;font-size:11px;font-weight:700;color:inherit;text-decoration:underline;cursor:pointer">${esc(i18t('me_term_use',{d:pretty(ck.expected)}))}</button>
  ...
  const spanLine = k => { const q=spans[k]; if(!q) return '';
    ...
    if(!t) return '';
    return `<span style="display:block;margin-top:3px;font-size:11px;line-height:1.45;color:var(--color-neutral-600)">found: <i>“${esc(t)}”</i></span>`; };
  const field = f => {
  ...
  openModal(`
    <div class="p-6 max-w-lg">
      ...
      ${opts.ocrNotice?`<div style="display:flex;align-items:flex-start;gap:8px;border:1px solid var(--st-amber-line);background:var(--st-amber-bg);color:var(--st-amber-fg);border-radius:0;padding:8px 11px;font-size:12px;line-height:1.55;margin:-8px 0 14px">
