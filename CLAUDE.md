...
THE TRACKED CHANGES HEAD SITS ON THE COLUMN'S OWN GROUND (added 2026-08-10, "make the top have a more professional look and balanced"). .nego-index-head paints var(--n-paper) — the ROOM's token, which resolves white — so on the workbench the head was a white band lying across the grey .nego-pane.index, caption jammed against one end and a heavier grey pill against the other. Both go transparent: the change column "sits straight on the page", which is the rule stated at .rl-col and had never been true of this one. The count is now the caption's own size and only takes the accent tint (.rl-idx-n.is-live) when something is actually on the table. Tests: f175.
