...
function openWizard(preTid){
  ...
  const renderStep=()=>{
    if(!tid){
      ...
      const rowCard=r=>`<button data-wz-${r.pick}="${esc2(r.id)}" style="text-align:left;border:${r.std?'1.5px solid var(--color-accent)':'1px solid var(--color-divider)'};background:${r.std?'var(--color-accent-100)':'var(--color-surface)'};border-radius:0;padding:var(--s-3);cursor:pointer;">
        <span style="display:flex;align-items:center;gap:var(--s-2);"><span style="width:28px;height:28px;display:grid;place-items:center;border-radius:0;background:${r.std?'var(--color-accent)':'var(--color-accent-100)'};color:${r.std?'#fff':'var(--color-accent)'};flex:none;">${icon(r.ic,'w-3.5 h-3.5')}</span>
      ...
      const streamCard=k=>{
        ...
        return `<button data-wz-stream="${esc2(k)}" style="text-align:left;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:0;padding:var(--s-3);cursor:pointer;border-left:3px solid ${fColor(k)};">
          <span style="display:flex;align-items:center;gap:var(--s-2);"><span style="width:28px;height:28px;display:grid;place-items:center;border-radius:0;background:color-mix(in srgb,${fColor(k)} 15%,transparent);color:${fColor(k)};flex:none;">${icon(fIcon(k),'w-3.5 h-3.5')}</span>
      ...
      openModal(`<div style="padding:22px var(--s-6);">
        ...
        <div id="wz-pick" class="scroll-thin" style="max-height:62vh;overflow-y:auto;">
          ...
          ${admin?`<label style="display:flex;align-items:center;gap:var(--s-2);margin:0 0 var(--s-3);font-size:var(--t-label);color:var(--color-neutral-600)">${i18t('wz_line_of_business')}
            <select id="wz-industry" style="border:1px solid var(--color-divider);background:var(--color-surface);border-radius:0;padding:var(--s-1) var(--s-2);font:inherit;font-size:var(--t-meta);color:inherit">
          ...
          <input id="wz-search" type="search" placeholder="${i18t('wz_search_all')}" autocomplete="off"
            style="width:100%;border:1px solid var(--color-divider);background:var(--color-bg);border-radius:0;padding:var(--s-2) var(--s-3);font:inherit;font-size:var(--t-body);color:inherit;outline:none;margin-bottom:10px"/>
    ...
    const input=v=>{ const id='wz-'+String(v.key).replace(/[:]/g,'_');
      ...
      if(v.type==='select') return `<label style="display:block;">${lbl}
        <select id="${id}" style="width:100%;min-height:36px;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:0;padding:7px 11px;font-size:var(--t-body);color:var(--color-text);outline:none;">
      ...
      return `<label style="display:block;">${lbl}
        <input id="${id}" type="${it}" value="${String(v.def||'').replace(/"/g,'&quot;')}" placeholder="${v.ph||''}" style="width:100%;min-height:36px;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:0;padding:7px 11px;font-size:var(--t-body);font-family:var(--font-body);color:var(--color-text);outline:none;"/></label>`; };
    openModal(`<div style="padding:22px var(--s-6);">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--s-3);">${vars.map(input).join('')}
        ...
        <label style="display:block;">
          ...
          <input id="wz-cpemail" type="email" placeholder="${(typeof jxEg==='function'&&jxEg('theirEmail'))||'them@company.co.ke'}" style="width:100%;min-height:36px;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:0;padding:7px 11px;font-size:var(--t-body);font-family:var(--font-body);color:var(--color-text);outline:none;"/></label>
      </div>
