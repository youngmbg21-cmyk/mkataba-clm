...
const _diffLegend = `<span style="display:inline-flex;align-items:center;gap:var(--s-2)"><ins style="background:var(--st-green-bg);color:var(--st-green-fg);text-decoration:none;border-radius:0;padding:0 var(--s-1)">added</ins><del style="background:var(--st-ruby-bg);color:var(--st-ruby-fg);border-radius:0;padding:0 var(--s-1)">removed</del></span>`;
const _diffBox = (a,b)=>`<div class="scroll-thin" style="border:1px solid var(--color-divider);border-radius:0;background:var(--color-surface);padding:14px var(--s-4);font-size:var(--t-body);line-height:1.85;color:var(--color-doc-text);max-height:56vh;overflow-y:auto;white-space:pre-wrap;font-family:var(--font-body)">${diffHtml(a,b)}</div>`;
const _statLine = (st)=>`<span style="font-weight:var(--w-strong);color:var(--st-green-fg)">+${st.add}</span> added · <span style="font-weight:var(--w-strong);color:var(--st-ruby-fg)">−${st.del}</span> removed`;

function openDiffModal(aText, bText, labelA, labelB){
  const st=diffStats(aText,bText);
  openModal(`
    <div style="padding:20px 22px">
      <div style="display:flex;align-items:center;gap:var(--s-2);margin-bottom:var(--s-1)"><span style="color:var(--color-accent)">${icon('history','w-4 h-4')}</span>
        <h3 style="font-family:var(--font-heading);font-weight:var(--w-strong);font-size:var(--t-page);margin:0">Compare ${labelA} → ${labelB}</h3></div>
      <p style="font-size:var(--t-meta);color:var(--color-neutral-600);margin:0 0 var(--s-3);display:flex;flex-wrap:wrap;gap:10px;align-items:center">${_statLine(st)} · ${_diffLegend}</p>
...
function openCompareModal(c){
  ...
  if(items.length<2){
    openModal(`
      <div style="padding:20px 22px">
        <div style="display:flex;align-items:center;gap:var(--s-2);margin-bottom:6px"><span style="color:var(--color-accent)">${icon('history','w-4 h-4')}</span>
          <h3 style="font-family:var(--font-heading);font-weight:var(--w-strong);font-size:var(--t-page);margin:0">${i18t('ve_compare_versions')}</h3></div>
        <p style="font-size:var(--t-body);color:var(--color-neutral-700);line-height:1.6;margin:0">There's only one version of this contract so far, so there's nothing to compare yet. New versions are captured automatically when a <b>${i18t('ve_cp_redline_accepted')}</b> and at <b>signing</b>. ${canSnap?'Capture a snapshot now, make your edits, then compare to see exactly what changed.':''}</p>
  ...
  const selStyle='font:inherit;font-size:var(--t-body);border:1px solid var(--color-divider);background:var(--color-surface);padding:6px var(--s-2);border-radius:0;color:inherit;min-width:0;flex:1';
  ...
  const structuredBox=(aText,bText)=>{
    ...
    return `<div class="cmp-doc" style="font-size:var(--t-body);line-height:1.75;border:1px solid var(--color-divider);border-radius:0;background:var(--color-surface);color:var(--color-doc-text);padding:14px 18px;max-height:62vh;overflow:auto">${html}</div>`;
  };
  const foldUnchanged=()=>{
  ...
  const segBtn=(k,label,on)=>`<button data-cmp-mode="${k}" style="border:0;border-radius:0;padding:5px var(--s-3);font:inherit;font-size:var(--t-meta);font-weight:var(--w-strong);cursor:pointer;background:${on?'var(--accent-solid,var(--color-accent))':'none'};color:${on?'#fff':'var(--color-neutral-600)'}">${label}</button>`;
  openModal(`
    <div style="padding:20px 22px">
      ...
      <div id="cmp-pair-row" style="display:flex;align-items:center;gap:var(--s-2);margin-bottom:var(--s-2)">
      ...
      <div id="cmp-legend" style="font-size:var(--t-meta);color:var(--color-neutral-600);margin-bottom:10px"></div>
      <style>
        ...
        #cmp-out .cmp-fold{display:block;width:100%;text-align:center;border:1px dashed var(--color-divider);background:var(--color-neutral-100);color:var(--color-neutral-600);border-radius:0;font:inherit;font-size:var(--t-label);font-weight:var(--w-strong);padding:var(--s-1);margin:var(--s-2) 0;cursor:pointer}
      ...
      <div id="cmp-out" style="font-size:var(--t-meta)"><span style="color:var(--color-neutral-500)">${i18t('ve_pick_two')} <b>${i18t('ve_compare')}</b> ${i18t('ve_to_see_changes')}</span></div>
  ...
  const run=()=>{
    ...
    if(aText===bText){
      ...
      document.getElementById('cmp-out').innerHTML=fmtOnly
        ? `<div style="font-size:var(--t-meta);line-height:1.6;color:var(--color-neutral-700);border:1px solid var(--st-steel-line);background:var(--st-steel-bg);border-radius:0;padding:10px var(--s-3)"><b>${i18t('ve_formatting_changed')}</b> ${i18t('ve_the_wording_of')} <b>${a.short}</b> and <b>${b.short}</b> is word-for-word identical, but the document's structure is not — headings, emphasis, clause numbering, indentation or table layout differ. Word-level comparison has nothing to highlight; open the two versions to see the difference.</div>`
        : `<div style="font-size:var(--t-meta);color:var(--color-neutral-500)">${i18t('ve_identical')} <b>${a.short}</b> and <b>${b.short}</b>.</div>`;
      return; }
    const st=diffStats(aText,bText);
  ...
  const runCum=()=>{
    ...
    const trail=moved.length
      ? `<div style="font-size:var(--t-meta);line-height:1.9;color:var(--color-neutral-700);border:1px solid var(--color-divider);border-radius:0;padding:9px var(--s-3);margin-bottom:10px"><b>${i18t('ve_clauses_moved')}</b> ${moved.slice(0,8).map(m=>`${m.label} ×${m.n}`).join(' · ')}${moved.length>8?` · +${moved.length-8} more`:''}</div>`
      : '';
...
function reviewProposedRound(c, n){
  ...
  const blockRow=b=>{
    ...
    <div style="border:1px solid var(--color-divider);background:var(--color-surface);border-radius:0;padding:11px 13px">
      ${b.context?`<div style="font-size:var(--t-label);color:var(--color-neutral-500);margin-bottom:6px">…${e(b.context)}</div>`:''}
      <div style="font-size:var(--t-body);line-height:1.7">
      ...
      ${ask?`<div style="margin-top:var(--s-2);border-left:2px solid var(--color-accent-300);background:var(--st-steel-bg);border-radius:0;padding:7px 10px">
      ...
      <input data-reply="${b.id}" type="text" placeholder="${i18t('ve_your_reply_optional')}" style="width:100%;margin-top:7px;border:1px solid var(--color-divider);border-radius:0;padding:6px 9px;font:inherit;font-size:var(--t-meta);background:var(--color-bg);outline:none"/>
    </div>`;};
  openModal(`
    <div style="height:100%;display:flex;flex-direction:column;min-height:0">
      <div style="flex:none;padding:20px 26px 14px;border-bottom:1px solid var(--color-divider)">
        <div style="${COL}">
          <div style="display:flex;align-items:center;gap:9px;flex-wrap:wrap">
            ...
            <span style="font-size:var(--t-micro);font-weight:var(--w-title);letter-spacing:.09em;text-transform:uppercase;background:var(--st-amber-bg);color:var(--st-amber-fg);border-radius:0;padding:3px 9px">${i18t('ve_round_open',{n})}</span>
          </div>
      ...
      <div class="scroll-thin" style="flex:1;min-height:0;overflow-y:auto;padding:22px 26px;background:var(--color-bg)">
        <div style="${COL}">
          ...
          ${r.comment?`<div style="margin-top:14px;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:0;padding:var(--s-3) var(--s-4)">
          ...
          <label style="display:block;margin-top:14px">
            ...
            <textarea id="pr-reply" rows="2" placeholder="${e(i18t('ve_ph_reply'))}" style="width:100%;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:0;padding:9px 11px;font:inherit;font-size:var(--t-body);outline:none"></textarea>
          </label>
        </div>
      </div>
      <div style="flex:none;padding:14px 26px;border-top:1px solid var(--color-divider)">
        <div style="${COL};display:flex;align-items:center;gap:var(--s-2);flex-wrap:wrap">
          <span id="pr-summary" style="font-size:var(--t-meta);color:var(--color-neutral-600)"></span>
          <span style="flex:1"></span>
