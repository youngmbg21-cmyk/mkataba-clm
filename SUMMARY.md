...
HaTi could already turn a Word file into a reusable template. It can now do the
same with a **PDF, including a scan of a paper form**.

It is deliberately the *same door*, not a second product. You go to the same
place — Templates → New template → Upload a document — pick a PDF instead of a
Word file, and you land on the same confirmation screen you already know, with
the same list of detected fields to check. Everything after that point is
identical: the builder, the library, publishing, creating contracts.

HaTi does not try to pick a PDF apart. The file is handed to Claude, which looks
at the pages the way a person does and sends back the same structured field list
the Word route produces. (The original brief had HaTi turn each page into a
picture first; it turned out Claude accepts a PDF directly and does that itself,
so there are no picture files for HaTi to make, store and tidy up. Simpler, and
one less thing to go wrong.)

**Scans get extra care, because reading a photograph of paper involves more
guesswork than reading a Word file.** HaTi works out for itself whether a PDF is
a proper digital document or a scan, by checking whether it contains real text
or just pictures of text. If it is a scan:

- A banner appears on the confirmation screen telling you it was a scan and to
  check the number fields carefully.
- Every field that holds digits — ID numbers, KRA PINs, phone numbers, company
  registration numbers — is **never** marked high confidence, no matter how sure
  Claude was. Digits are where scanning mistakes hide: a 3 misread as an 8 in a
  KRA PIN looks perfectly normal on screen. The confirmation screen already
  draws your eye to anything that is not high confidence, so this puts the
  risky fields in front of you.

That rule is enforced by HaTi itself after the fact, not by asking Claude to be
modest. Asking would be advice; doing it is a guarantee.

**Refused before any money is spent:** password-protected PDFs, damaged files,
and anything over 30 pages (with a message suggesting you split it or upload the
Word version). The page limit is checked by the server from the file itself,
...
Three new PDF test documents live in `fixtures/`, generated by
`node fixtures/generators/genbrutpdf.js`:

| Fixture | End-to-end result |
|---|---|
| `brut-account-opening.pdf` (digital, 2 pages) | Passes — classified digital, digit fields keep their confidence, publishes and fills cleanly |
| `brut-account-opening-scanned.pdf` (image-only, 2 pages) | Passes — classified as a scan, banner flag set, all five digit fields capped to medium |
| `blanks-mixed.pdf` (underscores and `[INSERT …]`) | Passes |

**Tests: `f128` 15/15 new, the Phase A–D suite still green (40/40 across
f101–f106), and the whole suite 2198/2198 with no failures (plus all four Chromium checks green: 71/71, 22/22, 18/18, 19/19).** One existing test
was deliberately changed: `f105` used to assert
that PDFs are *rejected*, which was correct before this work and wrong the moment
it landed. It now asserts the narrower true thing — bytes that are not a readable
document of either kind are still refused, before any model call.

A caveat worth knowing: the scanned fixture is **generated, not scanned** — text
drawn as pixels, with speckle. That is enough to prove classification, the
confidence cap, the banner and the pipeline. It is not a substitute for printing
the form, scanning it, and seeing how well Claude reads real scanner output.

## What was built, briefly

- Two new nullable columns on `templates` (`source_type`, `page_count`). Nothing
  was backfilled; templates made before this feature read as "not a scan".
- Server-side PDF inspection with no new dependency — encryption, page count and
  digital-vs-scan classification, read from the bytes using the zlib already in
  the file.
- The upload route now branches on the file's real signature, sending a PDF as an
  attached document and a Word file exactly as before.
- The shared detection prompt, tool and sanitiser are reused whole, with a
  PDF-only addendum for reading order, ruled boxes, handwriting and half-legible
  labels. Both routes produce identical output because they share the sanitiser,
  not because two code paths agree by convention.

**Not built, deliberately:** the single retry the brief asked for. It is not in
either route today, because a retry is only safe if it fires on an unusable
answer and never on a refusal or a rate limit, and deciding that correctly needs
real failure modes rather than a stub. Existing behaviour is unchanged and still
safe — a bad answer saves a draft with an error note, the original file is stored
before the call, and the upload never crashes. `BUGLOG.md` explains the call.

---

# Templates page cards, and a clean contract to send out

## The templates page

Company standard templates now render as cards in the same grid as My
templates, instead of full-width table rows — same card shape, same spacing,
same reflow as the window narrows.

**This also fixed a real bug, not just the styling.** In the reported
screenshot the green "New contract" buttons were sitting below and to the left
of their rows, with a stray arrow underneath. The cause: each row was a button
with another button inside it. That is invalid HTML, and browsers repair it by
closing the outer button early — which threw the New-contract button and the
arrow out of the row and dropped them below. A card is an ordinary container,
so its buttons behave. A test now fails if a button is ever nested inside
another one here.

One deliberate difference from My templates: the coloured stripe down the left
edge shows **status** — green published, amber draft — rather than category. On
company templates, draft-versus-published is the thing you scan for, and a
draft that looks published is the mistake worth designing against.

## The exported contract

An executed contract was carrying three things that should not go to a
counterparty: a "HaTi · Contract Lifecycle" masthead above the customer's own
letterhead, a "Generated by HaTi CLM" line at the foot, and the full audit
trail appended at the end.

**There are now two exports.**

- **PDF** — the copy you send. Your branding, the wording, the signatures and
  the seal. Nothing identifying the software that produced it.
- **Record** — the copy you keep. The same document plus HaTi's seal and the
  audit trail. Only offered on contracts HaTi actually executed.

The audit trail was not deleted, deliberately. It is the evidence of when and
how someone signed, and the first time a counterparty disputes that, you will
want it. This is the same split DocuSign draws between an agreement and its
certificate of completion. The signatures stay on the copy you send, because
those are the contract, not a HaTi mark.

**One thing you have to fix yourself, because it is not ours.** The
"https://hati-clm.onrender.com", the date, and the "3/4" page numbers in the
