...
function discussBubbleHtml(m, mine){
  ...
  return `
    <div style="display:flex;flex-direction:column;gap:2px;align-items:${isMine ? 'flex-end' : 'flex-start'}">
      ...
      <div style="max-width:92%;border:1px solid ${internal ? 'rgba(138,106,42,.35)' : (isMine ? 'var(--color-divider)' : 'var(--color-accent-300)')};background:${internal ? 'rgba(245,158,11,.08)' : (isMine ? 'var(--color-bg)' : 'var(--color-accent-100)')};border-radius:0;padding:var(--s-2) 11px;font-size:var(--t-meta);line-height:1.55;color:var(--color-neutral-800);white-space:pre-wrap">${e(m.body)}</div>
    </div>`;
}
...
function discussPanelHtml(opts){
  ...
  const composer = disabled ? `
    <div style="border:1px solid var(--color-divider);background:var(--color-bg);border-radius:0;padding:9px var(--s-3);font-size:var(--t-meta);line-height:1.55;color:var(--color-neutral-600)">${e(disabledNote || 'This conversation is closed.')}</div>` : `
    <div style="border-top:1px solid var(--color-divider);padding-top:var(--s-3)">
      <label style="display:block;margin-bottom:7px">
        ...
        <select id="${idp}-topic" style="width:100%;font:inherit;font-size:var(--t-meta);border:1px solid var(--color-divider);background:var(--color-surface);border-radius:0;padding:7px 9px;color:inherit">${options}</select>
      </label>
      <textarea id="${idp}-body" rows="2" placeholder="${e(i18t('di_ph_would_you'))}" style="width:100%;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:0;padding:var(--s-2) 11px;font:inherit;font-size:var(--t-body);outline:none;resize:vertical"></textarea>
  ...
  return `
    <div id="${idp}-panel" style="border:1px solid var(--color-divider);background:var(--color-surface);border-radius:0;padding:14px 18px;margin:0 0 18px;box-shadow:var(--shadow-sm)">
      <div style="display:flex;align-items:center;gap:var(--s-2);margin-bottom:11px;flex-wrap:wrap">
        ...
        ${waiting ? `<span style="font-size:var(--t-micro);font-weight:var(--w-title);letter-spacing:.09em;text-transform:uppercase;background:var(--st-amber-bg);color:var(--st-amber-fg);border-radius:0;padding:2px 9px">${waiting} awaiting your reply</span>` : ''}
  ...
  return `${thread}
    <div style="display:flex;gap:6px;align-items:center;margin-top:var(--s-2);flex-wrap:wrap">
      <textarea data-point-body="${e(idp)}" class="chat-field" rows="1" placeholder="${e(placeholder || 'Reply on this point — a sentence, not a redraft')}"
        style="flex:1;min-width:150px;border:1px solid var(--color-divider);border-radius:0;padding:7px 10px;font:inherit;font-size:var(--t-meta);background:var(--color-surface);outline:none"></textarea>
  ...
  document.querySelectorAll('[data-point-send]').forEach(btn => {
    ...
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (out) out.innerHTML = `<div style="border:1px solid var(--st-ruby-line);background:var(--st-ruby-bg);border-radius:0;padding:7px 10px;font-size:var(--t-label);line-height:1.5;color:var(--st-ruby-fg)"><b>${i18t('di_not_sent')}</b> ${(window.esc ? esc(e.message) : e.message) || 'Something went wrong.'} Your reply is still in the box.</div>`;
...
function wireDiscussPanel(opts){
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (out) out.innerHTML = `<div style="border:1px solid var(--st-ruby-line);background:var(--st-ruby-bg);border-radius:0;padding:9px 11px;font-size:var(--t-meta);line-height:1.5;color:var(--st-ruby-fg)"><b>${i18t('di_not_sent')}</b> ${(window.esc ? esc(e.message) : e.message) || 'Something went wrong.'} Your message is still in the box.</div>`;
