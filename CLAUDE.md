...
Tests: f154 (the model, the gate, the wall, both renderers, the real payload), f155 (the notification route) f156 (the picker, and a guard against broken encoding) and f157 (the chosen subset, the two colours, the warning). NOT f152/f153 — those numbers were already taken by the counterparty-view and monthly-report tests.
