...
function pipeCard(c){
  ...
  return `
    <div data-card="${c.id}" class="q-card" style="background:var(--color-surface);border:1px solid var(--color-divider);border-left:4px solid ${folderColor(c)};border-radius:0;box-shadow:var(--shadow-sm);padding:11px var(--s-3);cursor:pointer;display:flex;flex-direction:column;gap:5px">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:6px">
        <span style="font-family:var(--font-mono);font-size:var(--t-label);color:var(--color-neutral-600)">${c.id}</span>
        <span style="background:${rp.bg};color:${rp.fg};font-size:var(--t-label);font-weight:var(--w-strong);letter-spacing:.03em;padding:2px var(--s-2);border-radius:0;font-variant-numeric:tabular-nums;flex:none">R ${r}</span>
      </div>
...
function pipeColumnInner(col, list){
  ...
  const more=list.length>PIPE_CAP?`<button data-pipe-more="${col.k}" style="width:100%;border:1px solid var(--color-divider);background:var(--color-surface);border-radius:0;padding:6px 9px;font:inherit;font-size:var(--t-label);font-weight:var(--w-body);color:var(--accent-ink-700);cursor:pointer">+${list.length-PIPE_CAP} more in Register →</button>`:'';
  const empty=list.length?'':`<div style="border:1px dashed var(--color-divider);border-radius:0;padding:22px 10px;text-align:center;font-size:var(--t-label);color:var(--color-neutral-500)">${i18t('queue_nothing_here')}</div>`;
...
function renderPipeline(){
  ...
  const columnsHtml=groups.map(g=>`
    <div style="min-width:0;display:flex;flex-direction:column;min-height:0">
      ...
      <div class="pipe-col scroll-thin" style="background:color-mix(in srgb,var(--color-accent) 6%,transparent);border:1px solid var(--color-divider);border-radius:0;padding:var(--s-2);display:flex;flex-direction:column;gap:var(--s-2);flex:1;min-height:0;overflow-y:auto">
