...
function docDesignHeaderHtml(b, c, opts = {}) {
  ...
  if (d.id === 'classic-letterhead') {
    ...
    return `<div data-doc-design="classic-letterhead" style="text-align:${align};padding-bottom:12px;margin-bottom:22px;border-bottom:4px double ${BR_RULE};font-family:${BR_SERIF};color:${BR_INK}">
      ...
      ${ident ? `<div style="font-size:11px;color:${BR_SOFT};letter-spacing:.05em;margin-top:2px">${ident}</div>` : ''}
    </div>`;
  }
    ...
    return `<div data-doc-design="modern-minimal" style="margin-bottom:26px;font-family:${BR_SANS};color:${BR_INK}">
      <div style="display:flex;align-items:flex-start;gap:14px">
        ...
        <div style="text-align:right;flex:none">
          ...
          ${ident ? `<div style="font-size:11px;color:${BR_SOFT};line-height:1.5">${ident}</div>` : ''}
        </div>
      </div>
  ...
  if (d.id === 'formal-legal') {
    const left = pos === 'top-left' ? logoTop
      : `<span style="font-size:11px;letter-spacing:.22em;color:${BR_SOFT};text-transform:uppercase">${name || '&nbsp;'}</span>`;
    const right = pos === 'top-left'
      ? `<span style="font-size:11px;letter-spacing:.22em;color:${BR_SOFT};text-transform:uppercase">${name || '&nbsp;'}</span>`
      : (logoTop || `<span style="font-size:11px;letter-spacing:.22em;color:${BR_SOFT}">&nbsp;</span>`);
    ...
    return `<div data-doc-design="formal-legal" style="margin-bottom:20px;font-family:${BR_SERIF};color:${BR_INK}">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding-bottom:9px;border-bottom:1px solid ${BR_RULE}">
        ${pos === 'top-center' ? `<span style="font-size:11px;letter-spacing:.22em;color:${BR_SOFT};text-transform:uppercase">${name || '&nbsp;'}</span><span style="font-size:11px;letter-spacing:.22em;color:${BR_SOFT}">${ident || '&nbsp;'}</span>` : `${left}${right}`}
      </div>
      ${pos !== 'top-center' && ident ? `<div style="font-size:11px;color:${BR_SOFT};margin-top:4px;letter-spacing:.04em">${ident}</div>` : ''}
    </div>`;
  }
    ...
    return `<div data-doc-design="bold-corporate" style="${margin}${pad}background:${brAccent(b)};color:#fff;font-family:${BR_SANS}">
      <div style="display:flex;flex-direction:${justify};align-items:center;gap:14px;${pos === 'top-center' ? 'justify-content:center;text-align:center;' : ''}">
        ...
        <div style="min-width:0">
          ...
          ${ident ? `<div style="font-size:11px;opacity:.85;margin-top:1px">${ident}</div>` : ''}
        </div>
      </div>
    </div>`;
  }
  ...
  if (d.id === 'modern-editorial') {
    ...
    const identity = `<div style="min-width:0">
        ${name ? `<div style="font-size:15px;font-weight:700;letter-spacing:.01em">${name}</div>` : ''}
        ${ident ? `<div style="font-size:11px;color:${BR_SOFT};margin-top:2px">${ident}</div>` : ''}
      </div>`;
  ...
  if (d.id === 'ceremonial') {
    ...
    return `<div data-doc-design="ceremonial" style="text-align:center;margin-bottom:22px;font-family:${BR_SERIF};color:${BR_INK}">
      ...
      ${ident ? `<div style="font-size:11px;letter-spacing:.14em;color:${BR_SOFT};margin-top:5px;text-transform:uppercase">${ident}</div>` : ''}
  ...
  if (d.id === 'facing-parties') {
    ...
    const metaLine = facts.length ? `<div style="display:flex;gap:22px;margin-top:12px;padding-top:10px;border-top:1px dashed #cfd8dc;font-size:11px;color:${BR_SOFT}">
      ...
      <div style="background:#f1f4f5;border:1px solid #e2e8ea;border-left:4px solid ${brAccent(b)};border-radius:0;padding:15px 19px">
        ...
        <div style="display:flex;gap:18px;align-items:center">
          <div style="flex:1;min-width:0">
            ...
            ${ident ? `<div style="font-size:11px;color:${BR_SOFT};margin-top:3px;line-height:1.5">${ident}</div>` : ''}
          </div>
          ...
          <div style="flex:1;min-width:0;text-align:right">
            ...
            <div style="font-size:11px;color:${BR_SOFT};margin-top:3px">as per the signature page</div>
          </div>` : ''}
        </div>
        ${metaLine}
      </div>
...
function docDesignFooterHtml(b, c) {
  ...
  if (d.id === 'classic-letterhead')
    return `<div style="margin-top:26px;padding-top:10px;border-top:1px solid ${BR_RULE};text-align:center;font-family:${BR_SERIF};font-size:11px;color:${BR_SOFT}">${logoFoot ? logoFoot + '<br>' : ''}${line}</div>`;
  if (d.id === 'formal-legal')
    return `<div style="margin-top:26px;padding-top:9px;border-top:1px solid ${BR_RULE};text-align:center;font-family:${BR_SERIF};font-size:11px;letter-spacing:.14em;text-transform:uppercase;color:${BR_SOFT}">${logoFoot ? logoFoot + '<br>' : ''}${line}</div>`;
  if (d.id === 'bold-corporate')
    return `<div style="margin-top:26px;padding-top:10px;border-top:3px solid ${brAccent(b)};display:flex;align-items:center;gap:10px;font-family:${BR_SANS};font-size:11px;color:${BR_SOFT}">${logoFoot}<span style="flex:1">${line}</span></div>`;
  if (d.id === 'ceremonial')
...
function docDesignCoverPageHtml(b, c) {
  ...
  return `<div data-doc-design-cover="1" style="page-break-after:always;font-family:${font};color:${BR_INK};padding-top:60px;text-align:center">
    <div style="font-size:11px;letter-spacing:.24em;text-transform:uppercase;color:${BR_SOFT};margin-bottom:18px">${BR_T('br_contract_record', 'Contract record')}</div>
